...
namespace avel_tests {
    ...
    TEST(Mask4x32i, Any_edge_cases) {
        EXPECT_FALSE(any(mask4x32i{false}));
        EXPECT_TRUE(any(mask4x32i{true}));
    }

    TEST(Mask4x32i, Any_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr4xb>();
            auto cnt = std::count(inputs.begin(), inputs.end(), true);
    ...
    TEST(Mask4x32i, All_edge_cases) {
        EXPECT_FALSE(all(mask4x32i{false}));
        EXPECT_TRUE(all(mask4x32i{true}));
    }

    TEST(Mask4x32i, All_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr4xb>();
            auto cnt = std::count(inputs.begin(), inputs.end(), true);
            ...
            EXPECT_EQ(cnt == vec4x32i::width, all(v));
        }
    }

    TEST(Mask4x32i, None_edge_cases) {
        EXPECT_FALSE(all(mask4x32i{false}));
        EXPECT_TRUE(all(mask4x32i{true}));
    }

    TEST(Mask4x32i, None_random) {
    ...
    TEST(Mask4x32i, Insert_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            EXPECT_TRUE(v == mask4x32i{inputs});
        }
    }
    ...
    TEST(Vec4x32i, Insert_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr4x32i>();
            vec4x32i v{random_val<vec4x32i::scalar>()};
    ...
    TEST(Vec4x32i, Div_uniform) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            vec4x32i::scalar quotient  = (numerator / denominator);
            vec4x32i::scalar remainder = (numerator % denominator);
            ...
            constexpr auto min_value = std::numeric_limits<vec4x32i::scalar>::min();
    ...
    TEST(Vec4x32i, Popcount_edge_cases) {
        vec4x32i v{0x0};
    ...
    TEST(Vec4x32i, Countl_sign_edge_cases) {
        EXPECT_TRUE(all(countl_sign(vec4x32i{0x00}) == vec4x32i{type_width<vec4x32i::scalar>::value - 1}));
        for (std::size_t i = 0; i < (type_width<vec4x32i::scalar>::value - 1); ++i) {
            vec4x32i::scalar v = vec4x32i::scalar(-1) << i;
            vec4x32i expected = vec4x32i(type_width<vec4x32i::scalar>::value - 1 - i);
            vec4x32i observed = countl_sign(vec4x32i{v});
            EXPECT_TRUE(all(expected == observed));
        }
    }

    TEST(Vec4x32i, Countl_sign_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr4x32i>();
