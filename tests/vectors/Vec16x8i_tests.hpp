...
namespace avel_tests {
    ...
    TEST(Mask16x8i, Any_edge_cases) {
        EXPECT_FALSE(any(mask16x8i{false}));
        EXPECT_TRUE(any(mask16x8i{true}));
    }

    TEST(Mask16x8i, Any_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr16xb>();
            auto cnt = std::count(inputs.begin(), inputs.end(), true);
    ...
    TEST(Mask16x8i, All_edge_cases) {
        EXPECT_FALSE(all(mask16x8i{false}));
        EXPECT_TRUE(all(mask16x8i{true}));
    }

    TEST(Mask16x8i, All_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr16xb>();
            auto cnt = std::count(inputs.begin(), inputs.end(), true);
            ...
            EXPECT_EQ(cnt == vec16x8i::width, all(v));
        }
    }

    TEST(Mask16x8i, None_edge_cases) {
        EXPECT_FALSE(all(mask16x8i{false}));
        EXPECT_TRUE(all(mask16x8i{true}));
    }

    TEST(Mask16x8i, None_random) {
    ...
    TEST(Mask16x8i, Insert_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            EXPECT_TRUE(v == mask16x8i{inputs});
        }
    }
    ...
    TEST(Vec16x8i, Countl_sign_edge_cases) {
        EXPECT_TRUE(all(countl_sign(vec16x8i{0x00}) == vec16x8i{type_width<vec16x8i::scalar>::value - 1}));
        for (std::size_t i = 0; i < (type_width<vec16x8i::scalar>::value - 1); ++i) {
            vec16x8i::scalar v = vec16x8i::scalar(-1) << i;
            vec16x8i expected = vec16x8i(type_width<vec16x8i::scalar>::value - 1 - i);
            vec16x8i observed = countl_sign(vec16x8i{v});
            EXPECT_TRUE(all(expected == observed));
        }
    }

    TEST(Vec16x8i, Countl_sign_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr16x8i>();
