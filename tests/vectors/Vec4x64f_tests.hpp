...
namespace avel_tests {
    ...
    TEST(Mask4x64f, Any_edge_cases) {
        EXPECT_FALSE(any(mask4x64f{false}));
        EXPECT_TRUE(any(mask4x64f{true}));
    }

    TEST(Mask4x64f, Any_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr4xb>();
            auto cnt = std::count(inputs.begin(), inputs.end(), true);
    ...
    TEST(Mask4x64f, All_edge_cases) {
        EXPECT_FALSE(all(mask4x64f{false}));
        EXPECT_TRUE(all(mask4x64f{true}));
    }

    TEST(Mask4x64f, All_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr4xb>();
            auto cnt = std::count(inputs.begin(), inputs.end(), true);
            ...
            EXPECT_EQ(cnt == vec4x64f::width, all(v));
        }
    }

    TEST(Mask4x64f, None_edge_cases) {
        EXPECT_FALSE(all(mask4x64f{false}));
        EXPECT_TRUE(all(mask4x64f{true}));
    }

    TEST(Mask4x64f, None_random) {
    ...
    TEST(Mask4x64f, Insert_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            EXPECT_TRUE(v == mask4x64f{inputs});
        }
    }
    ...
    */
    ...
    TEST(Vec4x64f, fdim_edge_cases) {
        ...
        vec4x64f a{+std::numeric_limits<vec4x64f::scalar>::max()};
        vec4x64f b{-std::numeric_limits<vec4x64f::scalar>::max()};
        EXPECT_TRUE(avel::all(avel::isinf(avel::fdim(a, b))));
    }

    TEST(Vec4x64f, fdim_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr4x64f>();
            auto inputs1 = random_array<arr4x64f>();
    ...
    /*
    TEST(Vec4x64f, fmod_edge_cases) {
        EXPECT_TRUE(avel::all(avel::fmod(vec4x64f{+0.0f}, vec4x64f{1.0f}) == vec4x64f{+0.0f}));
        EXPECT_TRUE(avel::all(avel::fmod(vec4x64f{-0.0f}, vec4x64f{1.0f}) == vec4x64f{-0.0f}));

        EXPECT_TRUE(avel::all(avel::isnan(avel::fmod(vec4x64f{+INFINITY}, vec4x64f{1.0f}))));
        EXPECT_TRUE(avel::all(avel::isnan(avel::fmod(vec4x64f{-INFINITY}, vec4x64f{1.0f}))));

        EXPECT_TRUE(avel::all(avel::isnan(avel::fmod(vec4x64f{1.0f}, vec4x64f{+0.0f}))));
        EXPECT_TRUE(avel::all(avel::isnan(avel::fmod(vec4x64f{1.0f}, vec4x64f{-0.0f}))));

        EXPECT_TRUE(avel::all(vec4x64f{1.0f} == avel::fmod(vec4x64f{1.0f}, vec4x64f{+INFINITY})));
        EXPECT_TRUE(avel::all(vec4x64f{1.0f} == avel::fmod(vec4x64f{1.0f}, vec4x64f{-INFINITY})));

        EXPECT_TRUE(avel::all(avel::isnan(avel::fmod(vec4x64f{1.0f}, vec4x64f{NAN}))));
        EXPECT_TRUE(avel::all(avel::isnan(avel::fmod(vec4x64f{NAN}, vec4x64f{1.0f}))));
        EXPECT_TRUE(avel::all(avel::isnan(avel::fmod(vec4x64f{NAN}, vec4x64f{NAN}))));
    }

    TEST(Vec4x64f, fmod_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr4x64f>();
            auto inputs1 = random_array<arr4x64f>();
            ...
            vec4x64f results = avel::fmod(v0, v1);
