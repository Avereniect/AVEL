...
namespace avel_tests {
    ...
    TEST(Mask16x16i, Any_edge_cases) {
        EXPECT_FALSE(any(mask16x16i{false}));
        EXPECT_TRUE(any(mask16x16i{true}));
    }

    TEST(Mask16x16i, Any_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr16xb>();
            auto cnt = std::count(inputs.begin(), inputs.end(), true);
    ...
    TEST(Mask16x16i, All_edge_cases) {
        EXPECT_FALSE(all(mask16x16i{false}));
        EXPECT_TRUE(all(mask16x16i{true}));
    }

    TEST(Mask16x16i, All_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr16xb>();
            auto cnt = std::count(inputs.begin(), inputs.end(), true);
            ...
            EXPECT_EQ(cnt == vec16x16i::width, all(v));
        }
    }

    TEST(Mask16x16i, None_edge_cases) {
        EXPECT_FALSE(all(mask16x16i{false}));
        EXPECT_TRUE(all(mask16x16i{true}));
    }

    TEST(Mask16x16i, None_random) {
    ...
    TEST(Mask16x16i, Extract_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr16xb>();
            mask16x16i v{inputs};
    ...
    TEST(Mask16x16i, Insert_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            EXPECT_TRUE(v == mask16x16i{inputs});
        }
    }
    ...
    TEST(Vec16x16i, Set_bits_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr16xb>();
    ...
    TEST(Vec16x16i, Load_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            EXPECT_TRUE(all(load<vec16x16i, 0x0d>(inputs.data()) == load<vec16x16i>(inputs.data(), 0x0d)));
    ...
    TEST(Vec16x16i, Aligned_load_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            EXPECT_TRUE(all(aligned_load<vec16x16i, 0x0d>(inputs.data()) == aligned_load<vec16x16i>(inputs.data(), 0x0d)));
    ...
    TEST(Vec16x16i, Div_uniform) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            vec16x16i::scalar quotient  = (numerator / denominator);
            vec16x16i::scalar remainder = (numerator % denominator);
            ...
            constexpr auto min_value = std::numeric_limits<vec16x16i::scalar>::min();
    ...
    TEST(Vec16x16i, Popcount_edge_cases) {
        vec16x16i v{0x0};
    ...
    TEST(Vec16x16i, Countl_sign_edge_cases) {
        EXPECT_TRUE(all(countl_sign(vec16x16i{0x00}) == vec16x16i{type_width<vec16x16i::scalar>::value - 1}));
        for (std::size_t i = 0; i < (type_width<vec16x16i::scalar>::value - 1); ++i) {
            vec16x16i::scalar v = vec16x16i::scalar(-1) << i;
            vec16x16i expected = vec16x16i(type_width<vec16x16i::scalar>::value - 1 - i);
            vec16x16i observed = countl_sign(vec16x16i{v});
            EXPECT_TRUE(all(expected == observed));
        }
    }

    TEST(Vec16x16i, Countl_sign_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr16x16i>();
