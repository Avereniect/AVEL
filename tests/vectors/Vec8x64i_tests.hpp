...
namespace avel_tests {
    ...
    TEST(Vec8x64i, Isqrt_edge_cases) {
        vec8x64i v0{0x00};
        vec8x64i v1{0x01};
        vec8x64i v2{std::numeric_limits<vec8x64i::scalar>::max()};
        ...
        auto expected0 = v0;
        auto expected1 = v1;
        auto expected2 = vec8x64i(avel::isqrt(std::numeric_limits<vec8x64i::scalar>::max()));
    ...
    TEST(Vec8x64i, Isqrt_squares) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_squares_array<arr8x64i>();

            vec8x64i v1{inputs};
            vec8x64i v0 = v1 - vec8x64i{1};
            vec8x64i v2 = v1 + vec8x64i{1};
            ...
            arr8x64i expected0{};
            for (std::size_t j = 0; j < inputs.size(); ++j) {
                expected0[j] = isqrt(vec8x64i::scalar(inputs[j] - 1));
            }
            ...
            arr8x64i expected1{};
            for (std::size_t j = 0; j < inputs.size(); ++j) {
                expected1[j] = isqrt(vec8x64i::scalar(inputs[j] + 0));
            }

            EXPECT_TRUE(all(results1 == vec8x64i{expected1}));

            arr8x64i expected2{};
            for (std::size_t j = 0; j < inputs.size(); ++j) {
                expected2[j] = isqrt(vec8x64i::scalar(inputs[j] + 1));
            }

            EXPECT_TRUE(all(results2 == vec8x64i{expected2}));
        }
    }
