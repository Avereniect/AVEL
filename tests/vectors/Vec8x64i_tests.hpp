...
namespace avel_tests {
    ...
    TEST(Mask8x64i, Any_edge_cases) {
        EXPECT_FALSE(any(mask8x64i{false}));
        EXPECT_TRUE(any(mask8x64i{true}));
    }

    TEST(Mask8x64i, Any_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr8xb>();
            auto cnt = std::count(inputs.begin(), inputs.end(), true);
    ...
    TEST(Mask8x64i, All_edge_cases) {
        EXPECT_FALSE(all(mask8x64i{false}));
        EXPECT_TRUE(all(mask8x64i{true}));
    }

    TEST(Mask8x64i, All_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr8xb>();
            auto cnt = std::count(inputs.begin(), inputs.end(), true);
            ...
            EXPECT_EQ(cnt == vec8x64i::width, all(v));
        }
    }

    TEST(Mask8x64i, None_edge_cases) {
        EXPECT_FALSE(all(mask8x64i{false}));
        EXPECT_TRUE(all(mask8x64i{true}));
    }

    TEST(Mask8x64i, None_random) {
    ...
    TEST(Mask8x64i, Insert_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            EXPECT_TRUE(v == mask8x64i{inputs});
        }
    }
    ...
    TEST(Vec8x64i, Load_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            EXPECT_TRUE(all(load<vec8x64i, 0x00>(inputs.data()) == load<vec8x64i>(inputs.data(), 0x00)));
            EXPECT_TRUE(all(load<vec8x64i, 0x01>(inputs.data()) == load<vec8x64i>(inputs.data(), 0x01)));
            EXPECT_TRUE(all(load<vec8x64i, 0x02>(inputs.data()) == load<vec8x64i>(inputs.data(), 0x02)));
            EXPECT_TRUE(all(load<vec8x64i, 0x03>(inputs.data()) == load<vec8x64i>(inputs.data(), 0x03)));
            EXPECT_TRUE(all(load<vec8x64i, 0x04>(inputs.data()) == load<vec8x64i>(inputs.data(), 0x04)));
            EXPECT_TRUE(all(load<vec8x64i, 0x05>(inputs.data()) == load<vec8x64i>(inputs.data(), 0x05)));
            EXPECT_TRUE(all(load<vec8x64i, 0x06>(inputs.data()) == load<vec8x64i>(inputs.data(), 0x06)));
            EXPECT_TRUE(all(load<vec8x64i, 0x07>(inputs.data()) == load<vec8x64i>(inputs.data(), 0x07)));
            EXPECT_TRUE(all(load<vec8x64i, 0x08>(inputs.data()) == load<vec8x64i>(inputs.data(), 0x08)));
    ...
    TEST(Vec8x64i, Aligned_load_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            EXPECT_TRUE(all(aligned_load<vec8x64i, 0x00>(inputs.data()) == aligned_load<vec8x64i>(inputs.data(), 0x00)));
            EXPECT_TRUE(all(aligned_load<vec8x64i, 0x01>(inputs.data()) == aligned_load<vec8x64i>(inputs.data(), 0x01)));
            EXPECT_TRUE(all(aligned_load<vec8x64i, 0x02>(inputs.data()) == aligned_load<vec8x64i>(inputs.data(), 0x02)));
            EXPECT_TRUE(all(aligned_load<vec8x64i, 0x03>(inputs.data()) == aligned_load<vec8x64i>(inputs.data(), 0x03)));
            EXPECT_TRUE(all(aligned_load<vec8x64i, 0x04>(inputs.data()) == aligned_load<vec8x64i>(inputs.data(), 0x04)));
            EXPECT_TRUE(all(aligned_load<vec8x64i, 0x05>(inputs.data()) == aligned_load<vec8x64i>(inputs.data(), 0x05)));
            EXPECT_TRUE(all(aligned_load<vec8x64i, 0x06>(inputs.data()) == aligned_load<vec8x64i>(inputs.data(), 0x06)));
            EXPECT_TRUE(all(aligned_load<vec8x64i, 0x07>(inputs.data()) == aligned_load<vec8x64i>(inputs.data(), 0x07)));
            EXPECT_TRUE(all(aligned_load<vec8x64i, 0x08>(inputs.data()) == aligned_load<vec8x64i>(inputs.data(), 0x08)));
    ...
    TEST(Vec8x64i, Gather_random) {
        ...
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            EXPECT_TRUE(all(gather<vec8x64i, 0x00>(test_data.data(), indices) == gather<vec8x64i>(test_data.data(), indices, 0x00)));
            EXPECT_TRUE(all(gather<vec8x64i, 0x01>(test_data.data(), indices) == gather<vec8x64i>(test_data.data(), indices, 0x01)));
            EXPECT_TRUE(all(gather<vec8x64i, 0x02>(test_data.data(), indices) == gather<vec8x64i>(test_data.data(), indices, 0x02)));
            EXPECT_TRUE(all(gather<vec8x64i, 0x03>(test_data.data(), indices) == gather<vec8x64i>(test_data.data(), indices, 0x03)));
            EXPECT_TRUE(all(gather<vec8x64i, 0x04>(test_data.data(), indices) == gather<vec8x64i>(test_data.data(), indices, 0x04)));
            EXPECT_TRUE(all(gather<vec8x64i, 0x05>(test_data.data(), indices) == gather<vec8x64i>(test_data.data(), indices, 0x05)));
            EXPECT_TRUE(all(gather<vec8x64i, 0x06>(test_data.data(), indices) == gather<vec8x64i>(test_data.data(), indices, 0x06)));
            EXPECT_TRUE(all(gather<vec8x64i, 0x07>(test_data.data(), indices) == gather<vec8x64i>(test_data.data(), indices, 0x07)));
            EXPECT_TRUE(all(gather<vec8x64i, 0x08>(test_data.data(), indices) == gather<vec8x64i>(test_data.data(), indices, 0x08)));
    ...
    TEST(Vec8x64i, Div_uniform) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            vec8x64i::scalar quotient  = (numerator / denominator);
            vec8x64i::scalar remainder = (numerator % denominator);
            ...
            constexpr auto min_value = std::numeric_limits<vec8x64i::scalar>::min();
    ...
    TEST(Vec8x64i, Popcount_edge_cases) {
        vec8x64i v{0x0};
    ...
    TEST(Vec8x64i, Countl_sign_edge_cases) {
        EXPECT_TRUE(all(countl_sign(vec8x64i{0x00}) == vec8x64i{type_width<vec8x64i::scalar>::value - 1}));
        for (std::size_t i = 0; i < (type_width<vec8x64i::scalar>::value - 1); ++i) {
            vec8x64i::scalar v = vec8x64i::scalar(-1) << i;
            vec8x64i expected = vec8x64i(type_width<vec8x64i::scalar>::value - 1 - i);
            vec8x64i observed = countl_sign(vec8x64i{v});
            EXPECT_TRUE(all(expected == observed));
        }
    }

    TEST(Vec8x64i, Countl_sign_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr8x64i>();
