...
namespace avel_tests {
    ...
    TEST(Mask2x64i, Any_edge_cases) {
        EXPECT_FALSE(any(mask2x64i{false}));
        EXPECT_TRUE(any(mask2x64i{true}));
    }

    TEST(Mask2x64i, Any_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr2xb>();
            auto cnt = std::count(inputs.begin(), inputs.end(), true);
    ...
    TEST(Mask2x64i, All_edge_cases) {
        EXPECT_FALSE(all(mask2x64i{false}));
        EXPECT_TRUE(all(mask2x64i{true}));
    }

    TEST(Mask2x64i, All_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr2xb>();
            auto cnt = std::count(inputs.begin(), inputs.end(), true);
            ...
            EXPECT_EQ(cnt == vec2x64i::width, all(v));
        }
    }

    TEST(Mask2x64i, None_edge_cases) {
        EXPECT_FALSE(all(mask2x64i{false}));
        EXPECT_TRUE(all(mask2x64i{true}));
    }

    TEST(Mask2x64i, None_random) {
    ...
    TEST(Mask2x64i, Insert_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            EXPECT_TRUE(v == mask2x64i{inputs});
        }
    }
    ...
    TEST(Vec2x64i, Div_uniform) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            vec2x64i::scalar quotient  = (numerator / denominator);
            vec2x64i::scalar remainder = (numerator % denominator);
            ...
            constexpr auto min_value = std::numeric_limits<vec2x64i::scalar>::min();
    ...
    TEST(Vec2x64i, Popcount_edge_cases) {
        vec2x64i v{0x0};
