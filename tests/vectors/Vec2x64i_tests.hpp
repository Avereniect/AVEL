...
namespace avel_tests {
    ...
    TEST(Vec2x64i, Load_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            EXPECT_TRUE(all(load<vec2x64i, 0x00>(input_array0.data()) == load<vec2x64i>(input_array0.data(), 0x00)));
            EXPECT_TRUE(all(load<vec2x64i, 0x01>(input_array0.data()) == load<vec2x64i>(input_array0.data(), 0x01)));
            EXPECT_TRUE(all(load<vec2x64i, 0x02>(input_array0.data()) == load<vec2x64i>(input_array0.data(), 0x02)));
        }
    }

    TEST(Vec2x64i, Aligned_load_n_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            alignas(alignof(vec2x64i)) arr2x64i input_array{};
            for (std::size_t j = 0; j < input_array.size(); ++j) {
                input_array[j] = random64u();
            }
    ...
    TEST(Vec2x64i, Aligned_load_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            EXPECT_TRUE(all(aligned_load<vec2x64i, 0x00>(input_array0.data()) == aligned_load<vec2x64i>(input_array0.data(), 0x00)));
            EXPECT_TRUE(all(aligned_load<vec2x64i, 0x01>(input_array0.data()) == aligned_load<vec2x64i>(input_array0.data(), 0x01)));
            EXPECT_TRUE(all(aligned_load<vec2x64i, 0x02>(input_array0.data()) == aligned_load<vec2x64i>(input_array0.data(), 0x02)));
        }
    }
    ...
    TEST(Vec2x64i, Algined_store_n_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            arr2x64i input_array0{};
            for (std::size_t j = 0; j < input_array0.size(); ++j) {
                input_array0[j] = random64u();
