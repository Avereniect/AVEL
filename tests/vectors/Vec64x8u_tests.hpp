...
namespace avel_tests {
    ...
    TEST(Vec64x8u, Isqrt_edge_cases) {
        vec64x8u v0{0x00};
        vec64x8u v1{0x01};
        vec64x8u v2{std::numeric_limits<vec64x8u::scalar>::max()};
        ...
        auto expected0 = v0;
        auto expected1 = v1;
        auto expected2 = vec64x8u(avel::isqrt(std::numeric_limits<vec64x8u::scalar>::max()));
    ...
    TEST(Vec64x8u, Isqrt_squares) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            arr64x8u expected0{};
            for (std::size_t j = 0; j < inputs.size(); ++j) {
                expected0[j] = isqrt(vec64x8u::scalar(inputs[j] - 1));
            }
            ...
            arr64x8u expected1{};
            for (std::size_t j = 0; j < inputs.size(); ++j) {
                expected1[j] = isqrt(vec64x8u::scalar(inputs[j] + 0));
            }
            ...
            arr64x8u expected2{};
            for (std::size_t j = 0; j < inputs.size(); ++j) {
                expected2[j] = isqrt(vec64x8u::scalar(inputs[j] + 1));
            }
