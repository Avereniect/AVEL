...
namespace avel_tests {
    ...
    TEST(Mask16x32i, Any_edge_cases) {
        EXPECT_FALSE(any(mask16x32i{false}));
        EXPECT_TRUE(any(mask16x32i{true}));
    }

    TEST(Mask16x32i, Any_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr16xb>();
            auto cnt = std::count(inputs.begin(), inputs.end(), true);
    ...
    TEST(Mask16x32i, All_edge_cases) {
        EXPECT_FALSE(all(mask16x32i{false}));
        EXPECT_TRUE(all(mask16x32i{true}));
    }

    TEST(Mask16x32i, All_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr16xb>();
            auto cnt = std::count(inputs.begin(), inputs.end(), true);
            ...
            EXPECT_EQ(cnt == vec16x32i::width, all(v));
        }
    }

    TEST(Mask16x32i, None_edge_cases) {
        EXPECT_FALSE(all(mask16x32i{false}));
        EXPECT_TRUE(all(mask16x32i{true}));
    }

    TEST(Mask16x32i, None_random) {
    ...
    TEST(Mask16x32i, Extract_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr16xb>();
            mask16x32i v{inputs};
    ...
    TEST(Mask16x32i, Insert_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            EXPECT_TRUE(v == mask16x32i{inputs});
        }
    }
    ...
    TEST(Vec16x32i, Set_bits_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr16xb>();
    ...
    TEST(Vec16x32i, Load_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            EXPECT_TRUE(all(load<vec16x32i, 0x0d>(inputs.data()) == load<vec16x32i>(inputs.data(), 0x0d)));
    ...
    TEST(Vec16x32i, Aligned_load_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            EXPECT_TRUE(all(aligned_load<vec16x32i, 0x0d>(inputs.data()) == aligned_load<vec16x32i>(inputs.data(), 0x0d)));
    ...
    TEST(Vec16x32i, Div_uniform) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            vec16x32i::scalar quotient  = (numerator / denominator);
            vec16x32i::scalar remainder = (numerator % denominator);
            ...
            constexpr auto min_value = std::numeric_limits<vec16x32i::scalar>::min();
    ...
    TEST(Vec16x32i, Popcount_edge_cases) {
        vec16x32i v{0x0};
    ...
    TEST(Vec16x32i, Countl_sign_edge_cases) {
        EXPECT_TRUE(all(countl_sign(vec16x32i{0x00}) == vec16x32i{type_width<vec16x32i::scalar>::value - 1}));
        for (std::size_t i = 0; i < (type_width<vec16x32i::scalar>::value - 1); ++i) {
            vec16x32i::scalar v = vec16x32i::scalar(-1) << i;
            vec16x32i expected = vec16x32i(type_width<vec16x32i::scalar>::value - 1 - i);
            vec16x32i observed = countl_sign(vec16x32i{v});
            EXPECT_TRUE(all(expected == observed));
        }
    }

    TEST(Vec16x32i, Countl_sign_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr16x32i>();
