...
namespace avel_tests {
    ...
    TEST(Vec1x32i, Isqrt_edge_cases) {
        vec1x32i v0{0x00};
        vec1x32i v1{0x01};
        vec1x32i v2{std::numeric_limits<vec1x32i::scalar>::max()};
        ...
        auto expected0 = v0;
        auto expected1 = v1;
        auto expected2 = vec1x32i(avel::isqrt(std::numeric_limits<vec1x32i::scalar>::max()));
    ...
    TEST(Vec1x32i, Isqrt_squares) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            arr1x32i expected0{};
            for (std::size_t j = 0; j < inputs.size(); ++j) {
                expected0[j] = isqrt(vec1x32i::scalar(inputs[j] - 1));
            }
            ...
            arr1x32i expected1{};
            for (std::size_t j = 0; j < inputs.size(); ++j) {
                expected1[j] = isqrt(vec1x32i::scalar(inputs[j] + 0));
            }
            ...
            arr1x32i expected2{};
            for (std::size_t j = 0; j < inputs.size(); ++j) {
                expected2[j] = isqrt(vec1x32i::scalar(inputs[j] + 1));
            }
