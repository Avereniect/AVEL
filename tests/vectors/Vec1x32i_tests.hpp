...
namespace avel_tests {
    ...
    TEST(Mask1x32i, Any_edge_cases) {
        EXPECT_FALSE(any(mask1x32i{false}));
        EXPECT_TRUE(any(mask1x32i{true}));
    }

    TEST(Mask1x32i, Any_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr1xb>();
            auto cnt = std::count(inputs.begin(), inputs.end(), true);
    ...
    TEST(Mask1x32i, All_edge_cases) {
        EXPECT_FALSE(all(mask1x32i{false}));
        EXPECT_TRUE(all(mask1x32i{true}));
    }

    TEST(Mask1x32i, All_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr1xb>();
            auto cnt = std::count(inputs.begin(), inputs.end(), true);
            ...
            EXPECT_EQ(cnt == vec1x32i::width, all(v));
        }
    }

    TEST(Mask1x32i, None_edge_cases) {
        EXPECT_FALSE(all(mask1x32i{false}));
        EXPECT_TRUE(all(mask1x32i{true}));
    }

    TEST(Mask1x32i, None_random) {
    ...
    TEST(Mask1x32i, Insert_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            EXPECT_TRUE(v == mask1x32i{inputs});
        }
    }
    ...
    TEST(Vec1x32i, Div_uniform) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            vec1x32i::scalar quotient  = (numerator / denominator);
            vec1x32i::scalar remainder = (numerator % denominator);
            ...
            constexpr auto min_value = std::numeric_limits<vec1x32i::scalar>::min();
    ...
    TEST(Vec1x32i, Countl_sign_edge_cases) {
        EXPECT_TRUE(all(countl_sign(vec1x32i{0x00}) == vec1x32i{type_width<vec1x32i::scalar>::value - 1}));
        for (std::size_t i = 0; i < (type_width<vec1x32i::scalar>::value - 1); ++i) {
            vec1x32i::scalar v = vec1x32i::scalar(-1) << i;
            vec1x32i expected = vec1x32i(type_width<vec1x32i::scalar>::value - 1 - i);
            vec1x32i observed = countl_sign(vec1x32i{v});
            EXPECT_TRUE(all(expected == observed));
        }
    }

    TEST(Vec1x32i, Countl_sign_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr1x32i>();
