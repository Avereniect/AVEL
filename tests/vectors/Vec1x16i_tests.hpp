...
namespace avel_tests {
    ...
    TEST(Mask1x16i, Any_edge_cases) {
        EXPECT_FALSE(any(mask1x16i{false}));
        EXPECT_TRUE(any(mask1x16i{true}));
    }

    TEST(Mask1x16i, Any_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr1xb>();
            auto cnt = std::count(inputs.begin(), inputs.end(), true);
    ...
    TEST(Mask1x16i, All_edge_cases) {
        EXPECT_FALSE(all(mask1x16i{false}));
        EXPECT_TRUE(all(mask1x16i{true}));
    }

    TEST(Mask1x16i, All_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr1xb>();
            auto cnt = std::count(inputs.begin(), inputs.end(), true);
            ...
            EXPECT_EQ(cnt == vec1x16i::width, all(v));
        }
    }

    TEST(Mask1x16i, None_edge_cases) {
        EXPECT_FALSE(all(mask1x16i{false}));
        EXPECT_TRUE(all(mask1x16i{true}));
    }

    TEST(Mask1x16i, None_random) {
    ...
    TEST(Mask1x16i, Insert_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            EXPECT_TRUE(v == mask1x16i{inputs});
        }
    }
    ...
    TEST(Vec1x16i, Div_uniform) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            vec1x16i::scalar quotient  = (numerator / denominator);
            vec1x16i::scalar remainder = (numerator % denominator);
            ...
            constexpr auto min_value = std::numeric_limits<vec1x16i::scalar>::min();
    ...
    TEST(Vec1x16i, Popcount_edge_cases) {
        vec1x16i v{0x0};
    ...
    TEST(Vec1x16i, Countl_sign_edge_cases) {
        EXPECT_TRUE(all(countl_sign(vec1x16i{0x00}) == vec1x16i{type_width<vec1x16i::scalar>::value - 1}));
        for (std::size_t i = 0; i < (type_width<vec1x16i::scalar>::value - 1); ++i) {
            vec1x16i::scalar v = vec1x16i::scalar(-1) << i;
            vec1x16i expected = vec1x16i(type_width<vec1x16i::scalar>::value - 1 - i);
            vec1x16i observed = countl_sign(vec1x16i{v});
            EXPECT_TRUE(all(expected == observed));
        }
    }

    TEST(Vec1x16i, Countl_sign_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr1x16i>();
