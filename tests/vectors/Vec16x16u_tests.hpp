...
namespace avel_tests {
    ...
    TEST(Vec16x16u, Load_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            EXPECT_TRUE(all(load<vec16x16u, 0x0d>(inputs.data()) == load<vec16x16u>(inputs.data(), 0x0d)));
    ...
    TEST(Vec16x16u, Aligned_load_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            EXPECT_TRUE(all(aligned_load<vec16x16u, 0x0d>(inputs.data()) == aligned_load<vec16x16u>(inputs.data(), 0x0d)));
    ...
    TEST(Vec16x16u, Isqrt_edge_cases) {
        vec16x16u v0{0x00};
        vec16x16u v1{0x01};
        vec16x16u v2{std::numeric_limits<vec16x16u::scalar>::max()};
        ...
        auto expected0 = v0;
        auto expected1 = v1;
        auto expected2 = vec16x16u(avel::isqrt(std::numeric_limits<vec16x16u::scalar>::max()));
    ...
    TEST(Vec16x16u, Isqrt_squares) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_squares_array<arr16x16u>();

            vec16x16u v1{inputs};
            vec16x16u v0 = v1 - vec16x16u{1};
            vec16x16u v2 = v1 + vec16x16u{1};
            ...
            arr16x16u expected0{};
            for (std::size_t j = 0; j < inputs.size(); ++j) {
                expected0[j] = isqrt(vec16x16u::scalar(inputs[j] - 1));
            }

            EXPECT_TRUE(all(results0 == vec16x16u{expected0}));

            arr16x16u expected1{};
            for (std::size_t j = 0; j < inputs.size(); ++j) {
                expected1[j] = isqrt(vec16x16u::scalar(inputs[j] + 0));
            }

            EXPECT_TRUE(all(results1 == vec16x16u{expected1}));

            arr16x16u expected2{};
            for (std::size_t j = 0; j < inputs.size(); ++j) {
                expected2[j] = isqrt(vec16x16u::scalar(inputs[j] + 1));
            }

            EXPECT_TRUE(all(results2 == vec16x16u{expected2}));
        }
    }

    TEST(Vec16x16u, Isqrt) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr16x16u>();
