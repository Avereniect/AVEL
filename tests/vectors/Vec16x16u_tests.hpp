...
namespace avel_tests {
    ...
    TEST(Mask16x16u, Any_edge_cases) {
        EXPECT_FALSE(any(mask16x16u{false}));
        EXPECT_TRUE(any(mask16x16u{true}));
    }

    TEST(Mask16x16u, Any_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr16xb>();
            auto cnt = std::count(inputs.begin(), inputs.end(), true);
    ...
    TEST(Mask16x16u, All_edge_cases) {
        EXPECT_FALSE(all(mask16x16u{false}));
        EXPECT_TRUE(all(mask16x16u{true}));
    }

    TEST(Mask16x16u, All_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr16xb>();
            auto cnt = std::count(inputs.begin(), inputs.end(), true);
            ...
            EXPECT_EQ(cnt == vec16x16u::width, all(v));
        }
    }

    TEST(Mask16x16u, None_edge_cases) {
        EXPECT_FALSE(all(mask16x16u{false}));
        EXPECT_TRUE(all(mask16x16u{true}));
    }

    TEST(Mask16x16u, None_random) {
    ...
    TEST(Mask16x16u, Extract_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr16xb>();
            mask16x16u v{inputs};
    ...
    TEST(Mask16x16u, Insert_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            EXPECT_TRUE(v == mask16x16u{inputs});
        }
    }
