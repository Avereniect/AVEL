...
namespace avel_tests {
    ...
    TEST(Mask8x16i, Any_edge_cases) {
        EXPECT_FALSE(any(mask8x16i{false}));
        EXPECT_TRUE(any(mask8x16i{true}));
    }

    TEST(Mask8x16i, Any_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr8xb>();
            auto cnt = std::count(inputs.begin(), inputs.end(), true);
    ...
    TEST(Mask8x16i, All_edge_cases) {
        EXPECT_FALSE(all(mask8x16i{false}));
        EXPECT_TRUE(all(mask8x16i{true}));
    }

    TEST(Mask8x16i, All_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr8xb>();
            auto cnt = std::count(inputs.begin(), inputs.end(), true);
            ...
            EXPECT_EQ(cnt == vec8x16i::width, all(v));
        }
    }

    TEST(Mask8x16i, None_edge_cases) {
        EXPECT_FALSE(all(mask8x16i{false}));
        EXPECT_TRUE(all(mask8x16i{true}));
    }

    TEST(Mask8x16i, None_random) {
    ...
    TEST(Mask8x16i, Insert_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            EXPECT_TRUE(v == mask8x16i{inputs});
        }
    }
    ...
    TEST(Vec8x16i, Div_uniform) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            vec8x16i::scalar quotient  = (numerator / denominator);
            vec8x16i::scalar remainder = (numerator % denominator);
            ...
            constexpr auto min_value = std::numeric_limits<vec8x16i::scalar>::min();
    ...
    TEST(Vec8x16i, Popcount_edge_cases) {
        vec8x16i v{0x0};
    ...
    TEST(Vec8x16i, Countl_sign_edge_cases) {
        EXPECT_TRUE(all(countl_sign(vec8x16i{0x00}) == vec8x16i{type_width<vec8x16i::scalar>::value - 1}));
        for (std::size_t i = 0; i < (type_width<vec8x16i::scalar>::value - 1); ++i) {
            vec8x16i::scalar v = vec8x16i::scalar(-1) << i;
            vec8x16i expected = vec8x16i(type_width<vec8x16i::scalar>::value - 1 - i);
            vec8x16i observed = countl_sign(vec8x16i{v});
            EXPECT_TRUE(all(expected == observed));
        }
    }

    TEST(Vec8x16i, Countl_sign_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr8x16i>();
