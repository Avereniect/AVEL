...
namespace avel_tests {
    ...
    TEST(Mask8x16u, Any_edge_cases) {
        EXPECT_FALSE(any(mask8x16u{false}));
        EXPECT_TRUE(any(mask8x16u{true}));
    }

    TEST(Mask8x16u, Any_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr8xb>();
            auto cnt = std::count(inputs.begin(), inputs.end(), true);
    ...
    TEST(Mask8x16u, All_edge_cases) {
        EXPECT_FALSE(all(mask8x16u{false}));
        EXPECT_TRUE(all(mask8x16u{true}));
    }

    TEST(Mask8x16u, All_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr8xb>();
            auto cnt = std::count(inputs.begin(), inputs.end(), true);
            ...
            EXPECT_EQ(cnt == vec8x16u::width, all(v));
        }
    }

    TEST(Mask8x16u, None_edge_cases) {
        EXPECT_FALSE(all(mask8x16u{false}));
        EXPECT_TRUE(all(mask8x16u{true}));
    }

    TEST(Mask8x16u, None_random) {
    ...
    TEST(Mask8x16u, Insert_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            EXPECT_TRUE(v == mask8x16u{inputs});
        }
    }
