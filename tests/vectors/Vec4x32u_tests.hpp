...
namespace avel_tests {
    ...
    TEST(Vec4x32u, Isqrt_edge_cases) {
        vec4x32u v0{0x00};
        vec4x32u v1{0x01};
        vec4x32u v2{std::numeric_limits<vec4x32u::scalar>::max()};
        ...
        auto expected0 = v0;
        auto expected1 = v1;
        auto expected2 = vec4x32u(avel::isqrt(std::numeric_limits<vec4x32u::scalar>::max()));
    ...
    TEST(Vec4x32u, Isqrt_squares) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            arr4x32u expected0{};
            for (std::size_t j = 0; j < inputs.size(); ++j) {
                expected0[j] = isqrt(vec4x32u::scalar(inputs[j] - 1));
            }
            ...
            arr4x32u expected1{};
            for (std::size_t j = 0; j < inputs.size(); ++j) {
                expected1[j] = isqrt(vec4x32u::scalar(inputs[j] + 0));
            }
            ...
            arr4x32u expected2{};
            for (std::size_t j = 0; j < inputs.size(); ++j) {
                expected2[j] = isqrt(vec4x32u::scalar(inputs[j] + 1));
            }
