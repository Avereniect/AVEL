...
namespace avel_tests {
    ...
    TEST(Vec4x32u, Load_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            EXPECT_TRUE(all(load<vec4x32u, 0x00>(input_array0.data()) == load<vec4x32u>(input_array0.data(), 0x00)));
            EXPECT_TRUE(all(load<vec4x32u, 0x01>(input_array0.data()) == load<vec4x32u>(input_array0.data(), 0x01)));
            EXPECT_TRUE(all(load<vec4x32u, 0x02>(input_array0.data()) == load<vec4x32u>(input_array0.data(), 0x02)));
            EXPECT_TRUE(all(load<vec4x32u, 0x03>(input_array0.data()) == load<vec4x32u>(input_array0.data(), 0x03)));
            EXPECT_TRUE(all(load<vec4x32u, 0x04>(input_array0.data()) == load<vec4x32u>(input_array0.data(), 0x04)));
        }
    }

    TEST(Vec4x32u, Aligned_load_n_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            alignas(alignof(vec4x32u)) arr4x32u input_array{};
            for (std::size_t j = 0; j < input_array.size(); ++j) {
                input_array[j] = random32u();
            }

            for (std::uint32_t j = 0; j <= vec4x32u::width; ++j) {
                auto results = aligned_load<vec4x32u>(input_array.data(), j);
    ...
    TEST(Vec4x32u, Aligned_load_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            EXPECT_TRUE(all(aligned_load<vec4x32u, 0x00>(input_array0.data()) == aligned_load<vec4x32u>(input_array0.data(), 0x00)));
            EXPECT_TRUE(all(aligned_load<vec4x32u, 0x01>(input_array0.data()) == aligned_load<vec4x32u>(input_array0.data(), 0x01)));
            EXPECT_TRUE(all(aligned_load<vec4x32u, 0x02>(input_array0.data()) == aligned_load<vec4x32u>(input_array0.data(), 0x02)));
            EXPECT_TRUE(all(aligned_load<vec4x32u, 0x03>(input_array0.data()) == aligned_load<vec4x32u>(input_array0.data(), 0x03)));
            EXPECT_TRUE(all(aligned_load<vec4x32u, 0x04>(input_array0.data()) == aligned_load<vec4x32u>(input_array0.data(), 0x04)));
        }
    }
