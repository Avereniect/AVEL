...
namespace avel_tests {
    ...
    TEST(Vec32x16u, Set_bits_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr32xb>();
    ...
    TEST(Vec32x16u, Isqrt_edge_cases) {
        vec32x16u v0{0x00};
        vec32x16u v1{0x01};
        vec32x16u v2{std::numeric_limits<vec32x16u::scalar>::max()};
        ...
        auto expected0 = v0;
        auto expected1 = v1;
        auto expected2 = vec32x16u(avel::isqrt(std::numeric_limits<vec32x16u::scalar>::max()));
    ...
    TEST(Vec32x16u, Isqrt_squares) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_squares_array<arr32x16u>();

            vec32x16u v1{inputs};
            vec32x16u v0 = v1 - vec32x16u{1};
            vec32x16u v2 = v1 + vec32x16u{1};
            ...
            arr32x16u expected0{};
            for (std::size_t j = 0; j < inputs.size(); ++j) {
                expected0[j] = isqrt(vec32x16u::scalar(inputs[j] - 1));
            }

            EXPECT_TRUE(all(results0 == vec32x16u{expected0}));

            arr32x16u expected1{};
            for (std::size_t j = 0; j < inputs.size(); ++j) {
                expected1[j] = isqrt(vec32x16u::scalar(inputs[j] + 0));
            }

            EXPECT_TRUE(all(results1 == vec32x16u{expected1}));

            arr32x16u expected2{};
            for (std::size_t j = 0; j < inputs.size(); ++j) {
                expected2[j] = isqrt(vec32x16u::scalar(inputs[j] + 1));
            }

            EXPECT_TRUE(all(results2 == vec32x16u{expected2}));
        }
    }

    TEST(Vec32x16u, Isqrt) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr32x16u>();
