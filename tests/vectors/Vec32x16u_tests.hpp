...
namespace avel_tests {
    ...
    TEST(Mask32x16u, Any_edge_cases) {
        EXPECT_FALSE(any(mask32x16u{false}));
        EXPECT_TRUE(any(mask32x16u{true}));
    }

    TEST(Mask32x16u, Any_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr32xb>();
            auto cnt = std::count(inputs.begin(), inputs.end(), true);
    ...
    TEST(Mask32x16u, All_edge_cases) {
        EXPECT_FALSE(all(mask32x16u{false}));
        EXPECT_TRUE(all(mask32x16u{true}));
    }

    TEST(Mask32x16u, All_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr32xb>();
            auto cnt = std::count(inputs.begin(), inputs.end(), true);
            ...
            EXPECT_EQ(cnt == vec32x16u::width, all(v));
        }
    }

    TEST(Mask32x16u, None_edge_cases) {
        EXPECT_FALSE(all(mask32x16u{false}));
        EXPECT_TRUE(all(mask32x16u{true}));
    }

    TEST(Mask32x16u, None_random) {
    ...
    TEST(Mask32x16u, Extract_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr32xb>();
            mask32x16u v{inputs};
    ...
    TEST(Mask32x16u, Insert_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            EXPECT_TRUE(v == mask32x16u{inputs});
        }
    }
