...
namespace avel_tests {
    ...
    TEST(Mask32x16i, Any_edge_cases) {
        EXPECT_FALSE(any(mask32x16i{false}));
        EXPECT_TRUE(any(mask32x16i{true}));
    }

    TEST(Mask32x16i, Any_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr32xb>();
            auto cnt = std::count(inputs.begin(), inputs.end(), true);
    ...
    TEST(Mask32x16i, All_edge_cases) {
        EXPECT_FALSE(all(mask32x16i{false}));
        EXPECT_TRUE(all(mask32x16i{true}));
    }

    TEST(Mask32x16i, All_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr32xb>();
            auto cnt = std::count(inputs.begin(), inputs.end(), true);
            ...
            EXPECT_EQ(cnt == vec32x16i::width, all(v));
        }
    }

    TEST(Mask32x16i, None_edge_cases) {
        EXPECT_FALSE(all(mask32x16i{false}));
        EXPECT_TRUE(all(mask32x16i{true}));
    }

    TEST(Mask32x16i, None_random) {
    ...
    TEST(Mask32x16i, Extract_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr32xb>();
            mask32x16i v{inputs};
    ...
    TEST(Mask32x16i, Insert_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            EXPECT_TRUE(v == mask32x16i{inputs});
        }
    }
    ...
    TEST(Vec32x16i, Countl_sign_edge_cases) {
        EXPECT_TRUE(all(countl_sign(vec32x16i{0x00}) == vec32x16i{type_width<vec32x16i::scalar>::value - 1}));
        for (std::size_t i = 0; i < (type_width<vec32x16i::scalar>::value - 1); ++i) {
            vec32x16i::scalar v = vec32x16i::scalar(-1) << i;
            vec32x16i expected = vec32x16i(type_width<vec32x16i::scalar>::value - 1 - i);
            vec32x16i observed = countl_sign(vec32x16i{v});
            EXPECT_TRUE(all(expected == observed));
        }
    }

    TEST(Vec32x16i, Countl_sign_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr32x16i>();
