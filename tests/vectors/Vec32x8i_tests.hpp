...
namespace avel_tests {
    ...
    TEST(Vec32x8i, Div_uniform) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            vec32x8i::scalar quotient  = (numerator / denominator);
            vec32x8i::scalar remainder = (numerator % denominator);
            ...
            constexpr auto min_value = std::numeric_limits<vec32x8i::scalar>::min();
    ...
    TEST(Vec32x8i, Countl_sign_edge_cases) {
        EXPECT_TRUE(all(countl_sign(vec32x8i{0x00}) == vec32x8i{type_width<vec32x8i::scalar>::value - 1}));
        for (std::size_t i = 0; i < (type_width<vec32x8i::scalar>::value - 1); ++i) {
            vec32x8i::scalar v = vec32x8i::scalar(-1) << i;
            vec32x8i expected = vec32x8i(type_width<vec32x8i::scalar>::value - 1 - i);
            vec32x8i observed = countl_sign(vec32x8i{v});
            EXPECT_TRUE(all(expected == observed));
        }
    }

    TEST(Vec32x8i, Countl_sign_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr32x8i>();
