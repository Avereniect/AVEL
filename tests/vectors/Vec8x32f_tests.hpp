...
namespace avel_tests {
    ...
    */
    ...
    TEST(Vec8x32f, fdim_edge_cases) {
        ...
        vec8x32f a{+std::numeric_limits<vec8x32f::scalar>::max()};
        vec8x32f b{-std::numeric_limits<vec8x32f::scalar>::max()};
        EXPECT_TRUE(avel::all(avel::isinf(avel::fdim(a, b))));
    }

    TEST(Vec8x32f, fdim_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr8x32f>();
            auto inputs1 = random_array<arr8x32f>();
    ...
    /*
    TEST(Vec8x32f, fmod_edge_cases) {
        EXPECT_TRUE(avel::all(avel::fmod(vec8x32f{+0.0f}, vec8x32f{1.0f}) == vec8x32f{+0.0f}));
        EXPECT_TRUE(avel::all(avel::fmod(vec8x32f{-0.0f}, vec8x32f{1.0f}) == vec8x32f{-0.0f}));

        EXPECT_TRUE(avel::all(avel::isnan(avel::fmod(vec8x32f{+INFINITY}, vec8x32f{1.0f}))));
        EXPECT_TRUE(avel::all(avel::isnan(avel::fmod(vec8x32f{-INFINITY}, vec8x32f{1.0f}))));

        EXPECT_TRUE(avel::all(avel::isnan(avel::fmod(vec8x32f{1.0f}, vec8x32f{+0.0f}))));
        EXPECT_TRUE(avel::all(avel::isnan(avel::fmod(vec8x32f{1.0f}, vec8x32f{-0.0f}))));

        EXPECT_TRUE(avel::all(vec8x32f{1.0f} == avel::fmod(vec8x32f{1.0f}, vec8x32f{+INFINITY})));
        EXPECT_TRUE(avel::all(vec8x32f{1.0f} == avel::fmod(vec8x32f{1.0f}, vec8x32f{-INFINITY})));

        EXPECT_TRUE(avel::all(avel::isnan(avel::fmod(vec8x32f{1.0f}, vec8x32f{NAN}))));
        EXPECT_TRUE(avel::all(avel::isnan(avel::fmod(vec8x32f{NAN}, vec8x32f{1.0f}))));
        EXPECT_TRUE(avel::all(avel::isnan(avel::fmod(vec8x32f{NAN}, vec8x32f{NAN}))));
    }

    TEST(Vec8x32f, fmod_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr8x32f>();
            auto inputs1 = random_array<arr8x32f>();
            ...
            vec8x32f results = avel::fmod(v0, v1);
    ...
    */
    ...
    TEST(Vec8x32f, frac_edge_cases) {
        vec8x32f pos_zero{+0.0};
        vec8x32f neg_zero{-0.0};
