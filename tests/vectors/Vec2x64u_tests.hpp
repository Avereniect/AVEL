...
namespace avel_tests {
    ...
    TEST(Vec2x64u, Isqrt_edge_cases) {
        vec2x64u v0{0x00};
        vec2x64u v1{0x01};
        vec2x64u v2{std::numeric_limits<vec2x64u::scalar>::max()};
        ...
        auto expected0 = v0;
        auto expected1 = v1;
        auto expected2 = vec2x64u(avel::isqrt(std::numeric_limits<vec2x64u::scalar>::max()));
    ...
    TEST(Vec2x64u, Isqrt_squares) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_squares_array<arr2x64u>();

            vec2x64u v1{inputs};
            vec2x64u v0 = v1 - vec2x64u{1};
            vec2x64u v2 = v1 + vec2x64u{1};
            ...
            arr2x64u expected0{};
            for (std::size_t j = 0; j < inputs.size(); ++j) {
                expected0[j] = isqrt(vec2x64u::scalar(inputs[j] - 1));
            }

            EXPECT_TRUE(all(results0 == vec2x64u{expected0}));

            arr2x64u expected1{};
            for (std::size_t j = 0; j < inputs.size(); ++j) {
                expected1[j] = isqrt(vec2x64u::scalar(inputs[j] + 0));
            }

            EXPECT_TRUE(all(results1 == vec2x64u{expected1}));

            arr2x64u expected2{};
            for (std::size_t j = 0; j < inputs.size(); ++j) {
                expected2[j] = isqrt(vec2x64u::scalar(inputs[j] + 1));
            }

            EXPECT_TRUE(all(results2 == vec2x64u{expected2}));
        }
    }
    ...
    TEST(Vec2x64u, Bit_width_edge_cases) {
        ...
        for (std::size_t i = 0; i < type_width<vec2x64u::scalar>::value; ++i) {
            vec2x64u::scalar v = vec2x64u::scalar(1) << i;
