...
namespace avel_tests {
    ...
    TEST(Mask1x8u, Any_edge_cases) {
        EXPECT_FALSE(any(mask1x8u{false}));
        EXPECT_TRUE(any(mask1x8u{true}));
    }

    TEST(Mask1x8u, Any_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr1xb>();
            auto cnt = std::count(inputs.begin(), inputs.end(), true);
    ...
    TEST(Mask1x8u, All_edge_cases) {
        EXPECT_FALSE(all(mask1x8u{false}));
        EXPECT_TRUE(all(mask1x8u{true}));
    }

    TEST(Mask1x8u, All_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr1xb>();
            auto cnt = std::count(inputs.begin(), inputs.end(), true);
            ...
            EXPECT_EQ(cnt == vec1x8u::width, all(v));
        }
    }

    TEST(Mask1x8u, None_edge_cases) {
        EXPECT_FALSE(all(mask1x8u{false}));
        EXPECT_TRUE(all(mask1x8u{true}));
    }

    TEST(Mask1x8u, None_random) {
