...
namespace avel_tests {
    ...
    TEST(Vec1x8u, Isqrt_edge_cases) {
        vec1x8u v0{0x00};
        vec1x8u v1{0x01};
        vec1x8u v2{std::numeric_limits<vec1x8u::scalar>::max()};
        ...
        auto expected0 = v0;
        auto expected1 = v1;
        auto expected2 = vec1x8u(avel::isqrt(std::numeric_limits<vec1x8u::scalar>::max()));
    ...
    TEST(Vec1x8u, Isqrt_squares) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_squares_array<arr1x8u>();

            vec1x8u v1{inputs};
            vec1x8u v0 = v1 - vec1x8u{1};
            vec1x8u v2 = v1 + vec1x8u{1};
            ...
            arr1x8u expected0{};
            for (std::size_t j = 0; j < inputs.size(); ++j) {
                expected0[j] = isqrt(vec1x8u::scalar(inputs[j] - 1));
            }

            EXPECT_TRUE(all(results0 == vec1x8u{expected0}));

            arr1x8u expected1{};
            for (std::size_t j = 0; j < inputs.size(); ++j) {
                expected1[j] = isqrt(vec1x8u::scalar(inputs[j] + 0));
            }

            EXPECT_TRUE(all(results1 == vec1x8u{expected1}));

            arr1x8u expected2{};
            for (std::size_t j = 0; j < inputs.size(); ++j) {
                expected2[j] = isqrt(vec1x8u::scalar(inputs[j] + 1));
            }

            EXPECT_TRUE(all(results2 == vec1x8u{expected2}));
        }
    }
