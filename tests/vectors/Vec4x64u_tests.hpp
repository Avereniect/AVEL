...
namespace avel_tests {
    ...
    TEST(Vec4x64u, Load_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            EXPECT_TRUE(all(load<vec4x64u, 0x0>(inputs.data()) == load<vec4x64u>(inputs.data(), 0x0)));
            EXPECT_TRUE(all(load<vec4x64u, 0x1>(inputs.data()) == load<vec4x64u>(inputs.data(), 0x1)));
            EXPECT_TRUE(all(load<vec4x64u, 0x2>(inputs.data()) == load<vec4x64u>(inputs.data(), 0x2)));
            EXPECT_TRUE(all(load<vec4x64u, 0x3>(inputs.data()) == load<vec4x64u>(inputs.data(), 0x3)));
            EXPECT_TRUE(all(load<vec4x64u, 0x4>(inputs.data()) == load<vec4x64u>(inputs.data(), 0x4)));
        }
    }
    ...
    TEST(Vec4x64u, Aligned_load_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            EXPECT_TRUE(all(aligned_load<vec4x64u, 0x0>(inputs.data()) == aligned_load<vec4x64u>(inputs.data(), 0x0)));
            EXPECT_TRUE(all(aligned_load<vec4x64u, 0x1>(inputs.data()) == aligned_load<vec4x64u>(inputs.data(), 0x1)));
            EXPECT_TRUE(all(aligned_load<vec4x64u, 0x2>(inputs.data()) == aligned_load<vec4x64u>(inputs.data(), 0x2)));
            EXPECT_TRUE(all(aligned_load<vec4x64u, 0x3>(inputs.data()) == aligned_load<vec4x64u>(inputs.data(), 0x3)));
            EXPECT_TRUE(all(aligned_load<vec4x64u, 0x4>(inputs.data()) == aligned_load<vec4x64u>(inputs.data(), 0x4)));
        }
    }
    ...
    TEST(Vec4x64u, Gather_random) {
        ...
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            EXPECT_TRUE(all(gather<vec4x64u, 0x0>(test_data.data(), indices) == gather<vec4x64u>(test_data.data(), indices, 0x0)));
            EXPECT_TRUE(all(gather<vec4x64u, 0x1>(test_data.data(), indices) == gather<vec4x64u>(test_data.data(), indices, 0x1)));
            EXPECT_TRUE(all(gather<vec4x64u, 0x2>(test_data.data(), indices) == gather<vec4x64u>(test_data.data(), indices, 0x2)));
            EXPECT_TRUE(all(gather<vec4x64u, 0x3>(test_data.data(), indices) == gather<vec4x64u>(test_data.data(), indices, 0x3)));
            EXPECT_TRUE(all(gather<vec4x64u, 0x4>(test_data.data(), indices) == gather<vec4x64u>(test_data.data(), indices, 0x4)));
        }
    }
