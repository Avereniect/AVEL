...
namespace avel_tests {
    ...
    TEST(Vec32x8u, Load_n_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            for (std::size_t j = 0; j <= vec32x8u::width; ++j) {
                ...
                for (std::size_t k = 0; k < vec32x8u::width; k++) {
                    if (k < j) {
    ...
    TEST(Vec32x8u, Aligned_load_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            EXPECT_TRUE(all(aligned_load<vec32x8u, 0x09>(inputs.data()) == aligned_load<vec32x8u>(inputs.data(), 0x09)));
            ...
            EXPECT_TRUE(all(aligned_load<vec32x8u, 0x19>(inputs.data()) == aligned_load<vec32x8u>(inputs.data(), 0x19)));
    ...
    TEST(Vec32x8u, Isqrt_edge_cases) {
        vec32x8u v0{0x00};
        vec32x8u v1{0x01};
        vec32x8u v2{std::numeric_limits<vec32x8u::scalar>::max()};
        ...
        auto expected0 = v0;
        auto expected1 = v1;
        auto expected2 = vec32x8u(avel::isqrt(std::numeric_limits<vec32x8u::scalar>::max()));
    ...
    TEST(Vec32x8u, Isqrt_squares) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_squares_array<arr32x8u>();

            vec32x8u v1{inputs};
            vec32x8u v0 = v1 - vec32x8u{1};
            vec32x8u v2 = v1 + vec32x8u{1};
            ...
            arr32x8u expected0{};
            for (std::size_t j = 0; j < inputs.size(); ++j) {
                expected0[j] = isqrt(vec32x8u::scalar(inputs[j] - 1));
            }

            EXPECT_TRUE(all(results0 == vec32x8u{expected0}));

            arr32x8u expected1{};
            for (std::size_t j = 0; j < inputs.size(); ++j) {
                expected1[j] = isqrt(vec32x8u::scalar(inputs[j] + 0));
            }

            EXPECT_TRUE(all(results1 == vec32x8u{expected1}));

            arr32x8u expected2{};
            for (std::size_t j = 0; j < inputs.size(); ++j) {
                expected2[j] = isqrt(vec32x8u::scalar(inputs[j] + 1));
            }

            EXPECT_TRUE(all(results2 == vec32x8u{expected2}));
        }
    }
