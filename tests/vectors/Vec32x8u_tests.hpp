...
namespace avel_tests {
    ...
    TEST(Mask32x8u, Any_edge_cases) {
        EXPECT_FALSE(any(mask32x8u{false}));
        EXPECT_TRUE(any(mask32x8u{true}));
    }

    TEST(Mask32x8u, Any_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr32xb>();
            auto cnt = std::count(inputs.begin(), inputs.end(), true);
    ...
    TEST(Mask32x8u, All_edge_cases) {
        EXPECT_FALSE(all(mask32x8u{false}));
        EXPECT_TRUE(all(mask32x8u{true}));
    }

    TEST(Mask32x8u, All_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr32xb>();
            auto cnt = std::count(inputs.begin(), inputs.end(), true);
            ...
            EXPECT_EQ(cnt == vec32x8u::width, all(v));
        }
    }

    TEST(Mask32x8u, None_edge_cases) {
        EXPECT_FALSE(all(mask32x8u{false}));
        EXPECT_TRUE(all(mask32x8u{true}));
    }

    TEST(Mask32x8u, None_random) {
    ...
    TEST(Mask32x8u, Insert_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            EXPECT_TRUE(v == mask32x8u{inputs});
        }
    }
