...
namespace avel_tests {
    ...
    TEST(Vec16x8u, Isqrt_edge_cases) {
        vec16x8u v0{0x00};
        vec16x8u v1{0x01};
        vec16x8u v2{std::numeric_limits<vec16x8u::scalar>::max()};
        ...
        auto expected0 = v0;
        auto expected1 = v1;
        auto expected2 = vec16x8u(avel::isqrt(std::numeric_limits<vec16x8u::scalar>::max()));
    ...
    TEST(Vec16x8u, Isqrt_squares) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_squares_array<arr16x8u>();

            vec16x8u v1{inputs};
            vec16x8u v0 = v1 - vec16x8u{1};
            vec16x8u v2 = v1 + vec16x8u{1};
            ...
            arr16x8u expected0{};
            for (std::size_t j = 0; j < inputs.size(); ++j) {
                expected0[j] = isqrt(vec16x8u::scalar(inputs[j] - 1));
            }

            EXPECT_TRUE(all(results0 == vec16x8u{expected0}));

            arr16x8u expected1{};
            for (std::size_t j = 0; j < inputs.size(); ++j) {
                expected1[j] = isqrt(vec16x8u::scalar(inputs[j] + 0));
            }

            EXPECT_TRUE(all(results1 == vec16x8u{expected1}));

            arr16x8u expected2{};
            for (std::size_t j = 0; j < inputs.size(); ++j) {
                expected2[j] = isqrt(vec16x8u::scalar(inputs[j] + 1));
            }

            EXPECT_TRUE(all(results2 == vec16x8u{expected2}));
        }
    }
