...
namespace avel_tests {
    ...
    TEST(Vec8x32u, Gather_random) {
        ...
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            EXPECT_TRUE(all(gather<vec8x32u, 0x0>(test_data.data(), indices) == gather<vec8x32u>(test_data.data(), indices, 0x0)));
            EXPECT_TRUE(all(gather<vec8x32u, 0x1>(test_data.data(), indices) == gather<vec8x32u>(test_data.data(), indices, 0x1)));
            EXPECT_TRUE(all(gather<vec8x32u, 0x2>(test_data.data(), indices) == gather<vec8x32u>(test_data.data(), indices, 0x2)));
            EXPECT_TRUE(all(gather<vec8x32u, 0x3>(test_data.data(), indices) == gather<vec8x32u>(test_data.data(), indices, 0x3)));
            EXPECT_TRUE(all(gather<vec8x32u, 0x4>(test_data.data(), indices) == gather<vec8x32u>(test_data.data(), indices, 0x4)));
            EXPECT_TRUE(all(gather<vec8x32u, 0x5>(test_data.data(), indices) == gather<vec8x32u>(test_data.data(), indices, 0x5)));
            EXPECT_TRUE(all(gather<vec8x32u, 0x6>(test_data.data(), indices) == gather<vec8x32u>(test_data.data(), indices, 0x6)));
            EXPECT_TRUE(all(gather<vec8x32u, 0x7>(test_data.data(), indices) == gather<vec8x32u>(test_data.data(), indices, 0x7)));
        }
    }
    ...
    TEST(Vec8x32u, Isqrt_edge_cases) {
        vec8x32u v0{0x00};
        vec8x32u v1{0x01};
    ...
    TEST(Vec8x32u, Isqrt_squares) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_squares_array<arr8x32u>();

            vec8x32u v1{inputs};
            vec8x32u v0 = v1 - vec8x32u{1};
            vec8x32u v2 = v1 + vec8x32u{1};
            ...
            EXPECT_TRUE(all(results0 == vec8x32u{expected0}));
            ...
            EXPECT_TRUE(all(results1 == vec8x32u{expected1}));
            ...
            EXPECT_TRUE(all(results2 == vec8x32u{expected2}));
        }
    }
