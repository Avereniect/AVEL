...
namespace avel_tests {
    ...
    TEST(Mask1x64i, Construct_from_primitive) {
        mask1x64i mask{mask1x64i::primitive{}};
    ...
    TEST(Mask1x64i, Construct_from_bool) {
        mask1x64i mask0{false};
        EXPECT_FALSE(any(mask0));
        EXPECT_FALSE(all(mask0));
        EXPECT_TRUE(none(mask0));
        EXPECT_EQ(count(mask0), 0);

        mask1x64i mask1{true};
        EXPECT_TRUE(all(mask1));
        EXPECT_TRUE(any(mask1));
        EXPECT_FALSE(none(mask1));
        EXPECT_TRUE(count(mask1) == mask1x64i::width);
    }

    TEST(Mask1x64i, Construct_from_array) {
        arr1xb false_array{};
        std::fill_n(false_array.data(), mask1x64i::width, false);
        mask1x64i mask0{false_array};
        ...
        arr1xb true_array{};
        std::fill_n(true_array.data(), mask1x64i::width, true);
        mask1x64i mask1{true_array};
    ...
    TEST(Mask1x64i, Equality_comparison) {
        mask1x64i mask0{false};
        mask1x64i mask1{false};
        EXPECT_EQ(mask0, mask1);

        mask1x64i mask2{true};
        mask1x64i mask3{true};
    ...
    TEST(Mask1x64i, Equality_comparison_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1xb>();
            auto inputs1 = random_array<arr1xb>();
            inputs1[0] = !inputs0[0];

            mask1x64i mask0{inputs0};
            mask1x64i mask1{inputs0};
            mask1x64i mask2{inputs1};
    ...
    TEST(Mask1x64i, Inequality_comparison) {
        mask1x64i mask0{false};
        mask1x64i mask1{false};
    ...
    TEST(Mask1x64i, Inequality_comparison_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1xb>();
            auto inputs1 = random_array<arr1xb>();
            inputs1[0] = !inputs0[0];
    ...
    TEST(Mask1x64i, Bitwise_and_assignment_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1xb>();
            auto inputs1 = random_array<arr1xb>();
            ...
            mask1x64i expected_mask{expected};

            EXPECT_EQ(results, expected_mask);
        }
    }

    TEST(Mask1x64i, Bitwise_or_assignment_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1xb>();
            auto inputs1 = random_array<arr1xb>();
    ...
    TEST(Mask1x64i, Bitwise_xor_assignment_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1xb>();
            auto inputs1 = random_array<arr1xb>();
    ...
    TEST(Mask1x64i, Logical_negation_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1xb>();
    ...
    TEST(Mask1x64i, Bitwise_and_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1xb>();
            auto inputs1 = random_array<arr1xb>();
    ...
    TEST(Mask1x64i, Logical_and_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1xb>();
            auto inputs1 = random_array<arr1xb>();
    ...
    TEST(Mask1x64i, Bitwise_or_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1xb>();
            auto inputs1 = random_array<arr1xb>();
    ...
    TEST(Mask1x64i, Logical_or_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1xb>();
            auto inputs1 = random_array<arr1xb>();
    ...
    TEST(Mask1x64i, Bitwise_xor_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1xb>();
            auto inputs1 = random_array<arr1xb>();
    ...
    TEST(Mask1x64i, Any_edge_cases) {
        EXPECT_FALSE(any(mask1x64i{false}));
        EXPECT_TRUE(any(mask1x64i{true}));
    }

    TEST(Mask1x64i, Any_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr1xb>();
            auto cnt = std::count(inputs.begin(), inputs.end(), true);
    ...
    TEST(Mask1x64i, All_edge_cases) {
        EXPECT_FALSE(all(mask1x64i{false}));
        EXPECT_TRUE(all(mask1x64i{true}));
    }

    TEST(Mask1x64i, All_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr1xb>();
            auto cnt = std::count(inputs.begin(), inputs.end(), true);
            ...
            EXPECT_EQ(cnt == vec1x64i::width, all(v));
        }
    }

    TEST(Mask1x64i, None_edge_cases) {
        EXPECT_FALSE(all(mask1x64i{false}));
        EXPECT_TRUE(all(mask1x64i{true}));
    }

    TEST(Mask1x64i, None_random) {
    ...
    TEST(Mask1x64i, Insert_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            EXPECT_TRUE(v == mask1x64i{inputs});
        }
    }
    ...
    TEST(Vec1x64i, Construct_vector_from_mask_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr1xb>();
    ...
    TEST(Vec1x64i, Construct_from_scalar_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto x = random_val<vec1x64i::scalar>();
            vec1x64i results{x};

            arr1x64i arr;
            std::fill_n(arr.data(), vec1x64i::width, x);
            vec1x64i expected{arr};

            EXPECT_TRUE(all(results == expected));
        }
    }

    TEST(Vec1x64i, Construct_from_array_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto data = random_array<arr1x64i>();
    ...
    TEST(Vec1x64i, Assign_primitive) {
        vec1x64i::primitive x{};
        vec1x64i v{0x0F};
        v = x;
        vec1x64i expected{0x00};

        EXPECT_TRUE(all(v == expected));
    }

    TEST(Vec1x64i, Assign_scalar_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto scalar = random_val<vec1x64i::scalar>();
            vec1x64i v;
            v = scalar;
    ...
    TEST(Vec1x64i, Equality_comparison) {
        mask1x64i mask0{false};
        mask1x64i mask1{false};
        EXPECT_EQ(mask0, mask1);
    ...
    TEST(Vec1x64i, Equality_comparison_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1x64i>();
            auto inputs1 = random_array<arr1x64i>();
            inputs1[0] = ~inputs0[0];
    ...
    TEST(Vec1x64i, Inequality_comparison_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1x64i>();
            auto inputs1 = random_array<arr1x64i>();
            inputs1[0] = ~inputs0[0];
    ...
    TEST(Vec1x64i, Less_than_comparison_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1x64i>();
            auto inputs1 = random_array<arr1x64i>();
            ...
            EXPECT_EQ(results, mask1x64i{expected});
        }
    }

    TEST(Vec1x64i, Less_than_or_equal_comparison_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1x64i>();
            auto inputs1 = random_array<arr1x64i>();
    ...
    TEST(Vec1x64i, Greater_than_comparison_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1x64i>();
            auto inputs1 = random_array<arr1x64i>();
    ...
    TEST(Vec1x64i, Greater_than_or_equal_comparison_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1x64i>();
            auto inputs1 = random_array<arr1x64i>();
    ...
    TEST(Vec1x64i, Unary_plus) {
        auto inputs = random_array<arr1x64i>();
        const vec1x64i u{inputs};
        vec1x64i v{+u};

        EXPECT_TRUE(all(u == v));
    }

    TEST(Vec1x64i, Unary_minus) {
        auto inputs = random_array<arr1x64i>();
        const vec1x64i input{inputs};
    ...
    TEST(Vec1x64i, Plus_assignment_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1x64i>();
            auto inputs1 = random_array<arr1x64i>();
    ...
    TEST(Vec1x64i, Minus_assignment_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1x64i>();
            auto inputs1 = random_array<arr1x64i>();
    ...
    TEST(Vec1x64i, Times_assignment_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1x64i>();
            auto inputs1 = random_array<arr1x64i>();
    ...
    TEST(Vec1x64i, Div_assignment_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1x64i>();
            auto inputs1 = random_denominator_array<arr1x64i>();
    ...
    TEST(Vec1x64i, Mod_assignment_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1x64i>();
            auto inputs1 = random_denominator_array<arr1x64i>();
    ...
    TEST(Vec1x64i, Subtraction_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1x64i>();
            auto inputs1 = random_array<arr1x64i>();
    ...
    TEST(Vec1x64i, Multiplication_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1x64i>();
            auto inputs1 = random_array<arr1x64i>();
    ...
    TEST(Vec1x64i, Pre_increment_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr1x64i>();

            vec1x64i input{inputs};
            ++input;
    ...
    TEST(Vec1x64i, Post_increment_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr1x64i>();
    ...
    TEST(Vec1x64i, Pre_decrement_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr1x64i>();
    ...
    TEST(Vec1x64i, Post_decrement_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr1x64i>();
    ...
    TEST(Vec1x64i, Bitwise_and_assignment_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1x64i>();
            auto inputs1 = random_array<arr1x64i>();
    ...
    TEST(Vec1x64i, Bitwise_or_assignment_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1x64i>();
            auto inputs1 = random_array<arr1x64i>();
    ...
    TEST(Vec1x64i, Bitwise_xor_assignment_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1x64i>();
            auto inputs1 = random_array<arr1x64i>();
    ...
    TEST(Vec1x64i, Left_shift_by_scalar_assignment_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1x64i>();

            vec1x64i v0{inputs0};
            auto v1 = random_shift<vec1x64i::scalar>();
            ...
            arr1x64i expected{};
            for (std::size_t j = 0; j < inputs0.size(); ++j) {
                if (v1 >= type_width<vec1x64i::scalar>::value) {
                    expected[j] = 0x00;
                } else {
    ...
    TEST(Vec1x64i, Left_shift_by_vector_assignment_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1x64i>();
            auto inputs1 = random_shift_array<arr1x64i>();
            ...
            arr1x64i expected{};
            for (std::size_t j = 0; j < inputs0.size(); ++j) {
                if (inputs1[j] >= type_width<vec1x64i::scalar>::value) {
                    expected[j] = 0x00;
                } else {
                    expected[j] = inputs0[j] << inputs1[j];
                }
    ...
    TEST(Vec1x64i, Right_shift_by_scalar_assignment_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1x64i>();
    ...
    TEST(Vec1x64i, Right_shift_by_vector_assignment_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1x64i>();
            auto inputs1 = random_shift_array<arr1x64i>();
    ...
    TEST(Vec1x64i, Bitwise_negation_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr1x64i>();
    ...
    TEST(Vec1x64i, Left_shift_by_scalar_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1x64i>();
    ...
    TEST(Vec1x64i, Left_shift_by_vector_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1x64i>();
            auto inputs1 = random_shift_array<arr1x64i>();
    ...
    TEST(Vec1x64i, Right_shift_by_scalar_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1x64i>();
    ...
    TEST(Vec1x64i, Right_shift_by_vector_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1x64i>();
            auto inputs1 = random_shift_array<arr1x64i>();
    ...
    TEST(Vec1x64i, Conversion_to_mask) {
        vec1x64i vec{0};

        auto a = vec.operator mask1x64i();
        mask1x64i b{false};

        EXPECT_TRUE(a == b);
    }

    TEST(Vec1x64i, Conversion_to_mask_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr1x64i>();

            vec1x64i v{inputs};

            auto results = mask1x64i{v};
            ...
            EXPECT_TRUE(results == mask1x64i{expected});
        }
    }
    ...
    TEST(Vec1x64i, Insert_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr1x64i>();
            vec1x64i v{random_val<vec1x64i::scalar>()};

            v = insert<0x00>(v, inputs[0x00]);


            EXPECT_TRUE(all(v == vec1x64i{inputs}));
        }
    }
    ...
    TEST(Vec1x64i, Rotl_scalar_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1x64i>();
    ...
    TEST(Vec1x64i, Rotl_vector_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1x64i>();
            auto inputs1 = random_array<arr1x64i>();
    ...
    TEST(Vec1x64i, Rotr_scalar_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1x64i>();
    ...
    TEST(Vec1x64i, Rotr_vector_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1x64i>();
            auto inputs1 = random_array<arr1x64i>();
    ...
    TEST(Vec1x64i, Count_random) {
        auto predicate = [] (vec1x64i::scalar x) {
            return x != 0;
        };
    ...
    TEST(Vec1x64i, Any_edge_cases) {
        EXPECT_FALSE(any(vec1x64i{0x00}));
        EXPECT_TRUE(any(vec1x64i{0x01}));
    }

    TEST(Vec1x64i, Any_random) {
        auto predicate = [] (vec1x64i::scalar x) {
            return x != 0;
        };
    ...
    TEST(Vec1x64i, All_edge_cases) {
        EXPECT_FALSE(all(vec1x64i{0x00}));
        EXPECT_TRUE(all(vec1x64i{0x01}));
    }

    TEST(Vec1x64i, All_random) {
        auto predicate = [] (vec1x64i::scalar x) {
            return x != 0;
        };
        ...
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            EXPECT_EQ(cnt == vec1x64i::width, results);
        }
    }

    TEST(Vec1x64i, None_edge_cases) {
        EXPECT_TRUE(none(vec1x64i{0x00}));
        EXPECT_FALSE(none(vec1x64i{0x01}));
    }

    TEST(Vec1x64i, None_random) {
        auto predicate = [] (vec1x64i::scalar x) {
            return x != 0;
        };
    ...
    TEST(Vec1x64i, Set_bits_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1xb>();

            mask1x64i v0{inputs0};
    ...
    TEST(Vec1x64i, Blend_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1xb>();
            auto inputs1 = random_array<arr1x64i>();
            auto inputs2 = random_array<arr1x64i>();
    ...
    TEST(Vec1x64i, Minmax_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            EXPECT_TRUE(all(results[0] == vec1x64i{expected0}));
            EXPECT_TRUE(all(results[1] == vec1x64i{expected1}));
        }
    }

    TEST(Vec1x64i, Clamp_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs0 = random_array<arr1x64i>();
            auto inputs1 = random_array<arr1x64i>();
            auto inputs2 = random_array<arr1x64i>();
    ...
    TEST(Vec1x64i, Load_n_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr1x64i>();

            for (std::uint32_t j = 0; j < vec1x64i::width; ++j) {
                auto results = load<vec1x64i>(inputs.data(), j);

                auto loaded_data = to_array(results);
                for (std::uint32_t k = 0; k < vec1x64i::width; k++) {
                    if (k < j) {
    ...
    TEST(Vec1x64i, Load_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr1x64i>();

            EXPECT_TRUE(all(load<vec1x64i, 0x00>(inputs.data()) == load<vec1x64i>(inputs.data(), 0x00)));
            EXPECT_TRUE(all(load<vec1x64i, 0x01>(inputs.data()) == load<vec1x64i>(inputs.data(), 0x01)));

        }
    }

    TEST(Vec1x64i, Aligned_load_n_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            alignas(alignof(vec1x64i)) auto inputs = random_array<arr1x64i>();

            for (std::uint32_t j = 0; j < vec1x64i::width; ++j) {
                auto results = aligned_load<vec1x64i>(inputs.data(), j);
    ...
    TEST(Vec1x64i, Aligned_load_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            alignas(alignof(vec1x64i)) auto inputs = random_array<arr1x64i>();

            EXPECT_TRUE(all(aligned_load<vec1x64i, 0x00>(inputs.data()) == aligned_load<vec1x64i>(inputs.data(), 0x00)));
            EXPECT_TRUE(all(aligned_load<vec1x64i, 0x01>(inputs.data()) == aligned_load<vec1x64i>(inputs.data(), 0x01)));

        }
    }

    TEST(Vec1x64i, Gather_n_random) {
        static constexpr std::size_t test_data_size = 16 * 1024;

        std::vector<vec1x64i::scalar> test_data{};
        test_data.resize(test_data_size);
        for (auto& x : test_data) {
            x = random_val<vec1x64i::scalar>();
        }
        ...
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            for (std::size_t j = 0; j <= vec1x64i::width; ++j) {
                auto results = gather<vec1x64i>(test_data.data(), indices, j);

                auto loaded_data = to_array(results);
                for (std::size_t k = 0; k < vec1x64i::width; k++) {
                    if (k < j) {
    ...
    TEST(Vec1x64i, Gather_random) {
        ...
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            EXPECT_TRUE(all(gather<vec1x64i, 0x00>(test_data.data(), indices) == gather<vec1x64i>(test_data.data(), indices, 0x00)));
            EXPECT_TRUE(all(gather<vec1x64i, 0x01>(test_data.data(), indices) == gather<vec1x64i>(test_data.data(), indices, 0x01)));

        }
    }

    TEST(Vec1x64i, Store_n) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr1x64i>();
            ...
            for (std::size_t j = 0; j < vec1x64i::width; ++j) {
                vec1x64i::scalar arr[vec1x64i::width]{};
                store(arr, v, j);
    ...
    TEST(Vec1x64i, Aligned_store_n_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr1x64i>();
            ...
            for (std::size_t j = 0; j < vec1x64i::width; ++j) {
                alignas(alignof(vec1x64i)) vec1x64i::scalar arr[vec1x64i::width]{};
    ...
    TEST(Vec1x64i, Aligned_store_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr1x64i>();
    ...
    TEST(Vec1x64i, Scatter_n_random) {
        static constexpr std::size_t test_data_size = 16 * 1024;
        static constexpr std::size_t block_size = test_data_size / vec1x64i::width;

        std::vector<vec1x64i::scalar> test_data{};
        test_data.resize(test_data_size);

        for (std::size_t i = 0; i < iterations; ++i) {
            auto input_array0 = random_array<arr1x64i>();

            vec1x64i input0{input_array0};
    ...
    TEST(Vec1x64i, Div_uniform) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto numerator = random_val<vec1x64i::scalar>();
            auto denominator = random_denominator<vec1x64i::scalar>();

            vec1x64i::scalar quotient  = (numerator / denominator);
            vec1x64i::scalar remainder = (numerator % denominator);

            vec1x64i n{numerator};
            vec1x64i d{denominator};
            ...
            constexpr auto min_value = std::numeric_limits<vec1x64i::scalar>::min();
    ...
    TEST(Vec1x64i, Div_random) {
        for (unsigned i = 0; i < iterations; ++i) {
            auto numerators = random_array<arr1x64i>();
            auto denominators = random_denominator_array<arr1x64i>();
            ...
            vec1x64i n{numerators};
            vec1x64i d{denominators};
    ...
    TEST(Vec1x64i, Popcount_edge_cases) {
        vec1x64i v{0x0};
        vec1x64i c = popcount(v);
        EXPECT_TRUE(all(c == v));
    }

    TEST(Vec1x64i, Popcount_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr1x64i>();
    ...
    TEST(Vec1x64i, Countl_zero_edge_cases) {
        for (std::size_t i = 0; i < (type_width<vec1x64i::scalar>::value - 1); ++i) {
            vec1x64i::scalar v = (vec1x64i::scalar(0x1) << i) - 1;
            EXPECT_TRUE(all(countl_zero(vec1x64i{v}) == vec1x64i(type_width<vec1x64i::scalar>::value - i)));
        }
        EXPECT_TRUE(all(countl_zero(vec1x64i(-1)) == vec1x64i{0}));
    }

    TEST(Vec1x64i, Countl_zero_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr1x64i>();
    ...
    TEST(Vec1x64i, Countl_one_edge_cases) {
        EXPECT_TRUE(all(countl_one(vec1x64i{0x00}) == vec1x64i{0}));
        for (std::size_t i = 0; i < (type_width<vec1x64i::scalar>::value - 1); ++i) {
            vec1x64i::scalar v = vec1x64i::scalar(-1) << i;
            EXPECT_TRUE(all(countl_one(vec1x64i{v}) == vec1x64i(type_width<vec1x64i::scalar>::value - i)));
        }
    }

    TEST(Vec1x64i, Countl_one_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr1x64i>();
    ...
    TEST(Vec1x64i, Countr_zero_edge_cases) {
        for (std::size_t i = 0; i < (type_width<vec1x64i::scalar>::value - 1); ++i) {
            vec1x64i::scalar v = vec1x64i::scalar(-1) << i;
            EXPECT_TRUE(all(countr_zero(vec1x64i{v}) == vec1x64i(i)));
        }
        EXPECT_TRUE(all(countr_zero(vec1x64i{0x00}) == vec1x64i{type_width<vec1x64i::scalar>::value}));
    }

    TEST(Vec1x64i, Countr_zero_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr1x64i>();
    ...
    TEST(Vec1x64i, Countr_one_edge_cases) {
        EXPECT_TRUE(all(countr_one(vec1x64i{0x00}) == vec1x64i{0}));

        for (std::size_t i = 0; i < (type_width<vec1x64i::scalar>::value - 1); ++i) {
            vec1x64i::scalar v = (vec1x64i::scalar(1) << i) - 1;
            EXPECT_TRUE(all(countr_one(vec1x64i{v}) == vec1x64i(i)));
        }
        EXPECT_TRUE(all(countr_one(vec1x64i{vec1x64i::scalar(-1)}) == vec1x64i{type_width<vec1x64i::scalar>::value}));
    }

    TEST(Vec1x64i, Countr_one_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr1x64i>();
    ...
    TEST(Vec1x64i, Countl_sign_edge_cases) {
        EXPECT_TRUE(all(countl_sign(vec1x64i{0x00}) == vec1x64i{type_width<vec1x64i::scalar>::value - 1}));
        for (std::size_t i = 0; i < (type_width<vec1x64i::scalar>::value - 1); ++i) {
            vec1x64i::scalar v = vec1x64i::scalar(-1) << i;
            vec1x64i expected = vec1x64i(type_width<vec1x64i::scalar>::value - 1 - i);
            vec1x64i observed = countl_sign(vec1x64i{v});
            EXPECT_TRUE(all(expected == observed));
        }
    }

    TEST(Vec1x64i, Countl_sign_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr1x64i>();
    ...
    TEST(Vec1x64i, Has_single_bit_edge_cases) {
        EXPECT_TRUE(none(has_single_bit(vec1x64i{0x00})));

        for (std::size_t i = 0; i < (type_width<vec1x64i::scalar>::value - 1); ++i) {
            vec1x64i::scalar v = vec1x64i::scalar(1) << i;
            EXPECT_TRUE(all(has_single_bit(vec1x64i{v})));
        }
    }

    TEST(Vec1x64i, Has_single_bit_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr1x64i>();
