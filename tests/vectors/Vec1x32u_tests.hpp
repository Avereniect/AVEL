...
namespace avel_tests {
    ...
    TEST(Mask1x32u, Any_edge_cases) {
        EXPECT_FALSE(any(mask1x32u{false}));
        EXPECT_TRUE(any(mask1x32u{true}));
    }

    TEST(Mask1x32u, Any_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr1xb>();
            auto cnt = std::count(inputs.begin(), inputs.end(), true);
    ...
    TEST(Mask1x32u, All_edge_cases) {
        EXPECT_FALSE(all(mask1x32u{false}));
        EXPECT_TRUE(all(mask1x32u{true}));
    }

    TEST(Mask1x32u, All_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            auto inputs = random_array<arr1xb>();
            auto cnt = std::count(inputs.begin(), inputs.end(), true);
            ...
            EXPECT_EQ(cnt == vec1x32u::width, all(v));
        }
    }

    TEST(Mask1x32u, None_edge_cases) {
        EXPECT_FALSE(all(mask1x32u{false}));
        EXPECT_TRUE(all(mask1x32u{true}));
    }

    TEST(Mask1x32u, None_random) {
    ...
    TEST(Mask1x32u, Insert_random) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            EXPECT_TRUE(v == mask1x32u{inputs});
        }
    }
