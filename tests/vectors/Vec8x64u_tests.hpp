...
namespace avel_tests {
    ...
    TEST(Vec8x64u, Isqrt_edge_cases) {
        vec8x64u v0{0x00};
        vec8x64u v1{0x01};
        vec8x64u v2{std::numeric_limits<vec8x64u::scalar>::max()};
        ...
        auto expected0 = v0;
        auto expected1 = v1;
        auto expected2 = vec8x64u(avel::isqrt(std::numeric_limits<vec8x64u::scalar>::max()));
    ...
    TEST(Vec8x64u, Isqrt_squares) {
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            arr8x64u expected0{};
            for (std::size_t j = 0; j < inputs.size(); ++j) {
                expected0[j] = isqrt(vec8x64u::scalar(inputs[j] - 1));
            }
            ...
            arr8x64u expected1{};
            for (std::size_t j = 0; j < inputs.size(); ++j) {
                expected1[j] = isqrt(vec8x64u::scalar(inputs[j] + 0));
            }
            ...
            arr8x64u expected2{};
            for (std::size_t j = 0; j < inputs.size(); ++j) {
                expected2[j] = isqrt(vec8x64u::scalar(inputs[j] + 1));
            }
