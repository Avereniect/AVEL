...
namespace avel {
    ...
    div_type<vec1x64i> div(vec1x64i x, vec1x64i y);
    vec1x64i set_bits(mask1x64i m);
    ...
    [[nodiscard]]
    AVEL_FINL bool extract(mask1x64i m) {
    ...
    template<std::uint32_t N>
    [[nodiscard]]
    AVEL_FINL mask1x64i insert(mask1x64i m, bool b) {
        static_assert(N <= 1, "Specified index does not exist");
        typename std::enable_if<N <= 1, int>::type dummy_variable = 0;
    ...
    [[nodiscard]]
    AVEL_FINL vec1x64i set_bits(mask1x64i m) {
    ...
    AVEL_SIGNED_VECTOR_BIT_FUNCTIONS(vec1x64i, mask1x64i, vec1x64u)

    [[nodiscard]]
    AVEL_FINL vec1x64i countl_sign(vec1x64i v) {
