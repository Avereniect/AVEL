...
namespace avel {
    ...
    template<std::uint32_t N>
    AVEL_FINL mask8x64i insert(mask8x64i v, bool b) {
        static_assert(N < mask8x64i::width, "Specified index does not exist");
        typename std::enable_if<N < mask8x64i::width, int>::type dummy_variable = 0;

        return mask8x64i{insert<N>(mask8x64u{v}, b)};
    }
    ...
    AVEL_FINL vec8x64i isqrt(vec8x64i v) {
        auto clamped = max(v, vec8x64i{0x00});
        return vec8x64i{isqrt(vec8x64u{clamped})};
    }
