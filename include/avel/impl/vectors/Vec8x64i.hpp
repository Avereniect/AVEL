...
namespace avel {
    ...
    public:
        ...
        AVEL_FINL explicit Vector_mask(const arr8xb& arr) {
            ...
            auto array_data = _mm_cvtsi64_si128(bit_cast<std::uint64_t>(arr));
            auto expanded = _mm512_setzero_si512();
            expanded = _mm512_inserti64x4(expanded, _mm256_cvtepi8_epi32(array_data), 0x0);
    ...
    [[nodiscard]]
    AVEL_FINL bool extract(mask8x64i m) {
        static_assert(N < mask8x64i::width, "Specified index does not exist");
        typename std::enable_if<N < mask16x8i::width, int>::type dummy_variable = 0;

        return extract<N>(mask8x64u{m});
    }

    template<std::uint32_t N>
    [[nodiscard]]
    AVEL_FINL mask8x64i insert(mask8x64i m, bool b) {
        static_assert(N < mask8x64i::width, "Specified index does not exist");
        typename std::enable_if<N < mask8x64i::width, int>::type dummy_variable = 0;

        return mask8x64i{insert<N>(mask8x64u{m}, b)};
    }
    ...
    [[nodiscard]]
    AVEL_FINL vec8x64i countl_sign(vec8x64i v) {
        return avel::countl_zero((v ^ avel::bit_shift_right<1>(v))) - vec8x64i{1};
    }
