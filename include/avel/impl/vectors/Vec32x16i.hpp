...
namespace avel {
    ...
    div_type<vec32x16i> div(vec32x16i x, vec32x16i y);
    vec32x16i set_bits(mask32x16i m);
    ...
    [[nodiscard]]
    AVEL_FINL bool extract(mask32x16i m) {
        static_assert(N < mask32x16i::width, "Specified index does not exist");
        typename std::enable_if<N < mask32x16i::width, int>::type dummy_variable = 0;

        return extract<N>(mask32x16u{m});
    }

    template<std::uint32_t N>
    [[nodiscard]]
    AVEL_FINL mask32x16i insert(mask32x16i m, bool b) {
        static_assert(N < mask32x16i::width, "Specified index does not exist");
        typename std::enable_if<N < mask32x16i::width, int>::type dummy_variable = 0;

        return mask32x16i{insert<N>(mask32x16u{m}, b)};
    }
    ...
    [[nodiscard]]
    AVEL_FINL vec32x16i set_bits(mask32x16i m) {
        return vec32x16i{set_bits(mask32x16u{m})};
    }
    ...
    AVEL_FINL vec32x16i average(vec32x16i a, vec32x16i b) {
        #if defined(AVEL_AVX512BW)
        auto avg = (a & b) + ((a ^ b) >> 1);
        auto c = set_bits((a < -b) | (b == vec32x16i{std::int16_t(1 << 15)})) & (a ^ b) & vec32x16i{1};
        return avg + c;
        #endif
    }
    ...
    [[nodiscard]]
    AVEL_FINL vec32x16i countl_sign(vec32x16i v) {
        return avel::countl_zero((v ^ avel::bit_shift_right<1>(v))) - vec32x16i{1};
    }
