...
namespace avel {
    ...
    template<std::uint32_t N>
    AVEL_FINL mask8x16i insert(mask8x16i v, bool b) {
        static_assert(N < mask8x16i::width, "Specified index does not exist");
        typename std::enable_if<N < mask8x16i::width, int>::type dummy_variable = 0;

        return mask8x16i{insert<N>(mask8x16u{v}, b)};
    }
    ...
    public:
        ...
        AVEL_FINL explicit operator mask() const {
            ...
            #if defined(AVEL_NEON)
            return mask{vtstq_s16(content, content)};
            #endif
        }
    ...
    [[nodiscard]]
    AVEL_FINL vec8x16i isqrt(vec8x16i v) {
        return clear(v < vec8x16i{0x00}, vec8x16i{isqrt(vec8x16u{v})});
    }
