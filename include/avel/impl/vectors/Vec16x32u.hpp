...
namespace avel {
    ...
    template<std::uint32_t N>
    AVEL_FINL mask16x32u insert(mask16x32u v, bool b) {
        static_assert(N < mask16x32u::width, "Specified index does not exist");
        typename std::enable_if<N < mask16x32u::width, int>::type dummy_variable = 0;
    ...
    [[nodiscard]]
    AVEL_FINL vec16x32u isqrt(vec16x32u v) {
        #if defined(AVEL_AVX512F)
        auto clamped = min(v, vec16x32u{4294836225u});

        auto as_floats = _mm512_cvt_roundepu32_ps(decay(clamped), _MM_FROUND_TO_ZERO |_MM_FROUND_NO_EXC);
