...
namespace avel {
    ...
    AVEL_FINL vec2x64u isqrt(vec2x64u v) {
        #if defined(AVEL_AVX512VL) && defined(AVEL_AVX512DQ)
        auto clamped = min(v, vec2x64u{18446744065119617025ull});

        auto as_floats = _mm512_cvt_roundepu64_pd(_mm512_castsi128_si512(decay(clamped)), _MM_FROUND_TO_ZERO |_MM_FROUND_NO_EXC);
        auto fp_sqrts = _mm512_castpd512_pd128(_mm512_sqrt_round_pd(as_floats, _MM_FROUND_TO_ZERO |_MM_FROUND_NO_EXC));
        auto int_sqrts = _mm_cvttpd_epu64(fp_sqrts);
        auto adjusted_roots = _mm_add_epi64(int_sqrts, _mm_set1_epi64x(0x01));

        auto v_reconstructed = _mm_mullo_epi64(adjusted_roots, adjusted_roots);
        auto are_reconstructions_accurate = _mm_cmpge_epu64_mask(decay(v), v_reconstructed);

        auto ret = _mm_mask_blend_epi64(are_reconstructions_accurate, int_sqrts, adjusted_roots);
        return vec2x64u{ret};
