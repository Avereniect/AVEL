...
namespace avel {
    ...
    template<std::uint32_t N>
    [[nodiscard]]
    AVEL_FINL mask8x32i insert(mask8x32i m, bool b) {
        static_assert(N < mask8x32i::width, "Specified index does not exist");
        typename std::enable_if<N < mask8x32i::width, int>::type dummy_variable = 0;

        return mask8x32i{insert<N>(mask8x32u{m}, b)};
    }
    ...
    [[nodiscard]]
    AVEL_FINL vec8x32i countl_sign(vec8x32i v) {
        return avel::countl_zero((v ^ avel::bit_shift_right<1>(v))) - vec8x32i{1};
    }
