...
namespace avel {
    ...
    AVEL_FINL bool extract(mask8x32i m) {
        static_assert(N < mask8x32i::width, "Specified index does not exist");
        typename std::enable_if<N < mask8x32i::width, int>::type dummy_variable = 0;

        return extract<N>(mask8x32u{m});
    }

    template<std::uint32_t N>
    AVEL_FINL mask8x32i insert(mask8x32i v, bool b) {
        static_assert(N < mask8x32i::width, "Specified index does not exist");
        typename std::enable_if<N < mask8x32i::width, int>::type dummy_variable = 0;

        return mask8x32i{insert<N>(mask8x32u{v}, b)};
    }
    ...
    [[nodiscard]]
    AVEL_FINL vec8x32i isqrt(vec8x32i v) {
        #if defined(AVEL_AVX512VL)
        v = max(v, vec8x32i{0x00});

        auto as_floats = _mm512_cvt_roundepu32_ps(_mm512_castsi256_si512(decay(v)), _MM_FROUND_TO_ZERO |_MM_FROUND_NO_EXC);
