...
namespace avel {
    ...
    AVEL_FINL bool extract(mask1x16u v) {
    ...
    template<std::uint32_t N>
    AVEL_FINL mask1x16u insert(mask1x16u v, bool b) {
        static_assert(N < 1, "Specified index does not exist");
        typename std::enable_if<N < 1, int>::type dummy_variable = 0;
    ...
    template<std::uint32_t N>
    AVEL_FINL std::uint16_t extract(vec1x16u m) {
        static_assert(N < 1, "Specified index does not exist");
        typename std::enable_if<N < 1, int>::type dummy_variable = 0;
    ...
    [[nodiscard]]
    AVEL_FINL vec1x16u isqrt(vec1x16u v) {
        return vec1x16u{isqrt(decay(v))};
    }
