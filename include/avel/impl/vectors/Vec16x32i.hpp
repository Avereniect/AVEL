...
namespace avel {
    ...
    vec16x32i blend(vec16x32i a, vec16x32i b, mask4x32u m);

    std::array<std::int32_t, 16> to_array(vec16x32i v);
    ...
    template<>
    class alignas(AVEL_MASK_ALIGNMENT_16X32I) Vector_mask<std::int32_t, 16> {
    public:
    ...
    public:
       ...
       AVEL_FINL friend Vector operator*(Vector lhs, Vector rhs) {
        ...
        [[nodiscard]]
        AVEL_FINL friend Vector operator<<(Vector lhs, vec16x32u rhs) {
            lhs <<= rhs;
            return lhs;
        }
    ...
    AVEL_FINL void scatter(std::uint32_t* ptr, vec16x32i indices, vec16x32u v) {
        _mm512_i32scatter_epi32(ptr, indices, v, sizeof(std::uint32_t));
    }
