...
namespace avel {
    ...
    public:
        ...
        AVEL_FINL Denom4x64u& operator<<=(vec4x64u s) {
        ...
        [[nodiscard]]
        AVEL_FINL Denom4x64u operator<<(vec4x64u s) const {
            Denom4x64u ret = *this;
            ret <<= s;
        ...
        [[nodiscard]]
        AVEL_FINL vec4x64u value() const {
            return d;
        }
    ...
    private:
        ...
        static vec4x64u mulhi(vec4x64u x, vec4x64u y) {
            #if defined(AVEL_AVX2) && defined(AVEL_PCLMULQDQ)
            auto prod0 = _mm256_clmulepi64_epi128(decay(x), decay(y), 0x00);
            auto prod1 = _mm256_clmulepi64_epi128(decay(x), decay(y), 0x11);

            auto t1 = (x >> 63) & y;
            auto t2 = (y >> 63) & x;

            auto prod = _mm256_unpackhi_epi64(prod0, prod1);
            auto ret =  vec4x64u{prod} + t1 + t2;
            return ret;
