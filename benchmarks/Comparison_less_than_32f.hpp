...
namespace avel::benchmarks::comparison_equals_32f {
    ...
    mask4x32f vec4x32f_cmpps_impl(vec4x32f lhs, vec4x32f rhs) {
        return mask4x32f{_mm_cmplt_ps(decay(lhs), decay(rhs))};
    }
    ...
    mask4x32f vec4x32f_cmpltd_k_impl(vec4x32f lhs, vec4x32f rhs) {
        return mask4x32f{_mm_cmp_ps_mask(decay(lhs), decay(rhs), _CMP_LT_OS)};
    }

    auto vec4x32f_cmpltd_k = vector_comparison_test_bench<vec4x32f, vec4x32f_cmpltd_k_impl>;

    BENCHMARK(comparison_equals_32f::vec4x32f_cmpltd_k);
    ...
    mask8x32f vec8x32f_cmpps_impl(vec8x32f lhs, vec8x32f rhs) {
        return mask8x32f{_mm256_cmp_ps(decay(lhs), decay(rhs), _CMP_LT_OS)};
    }

    auto vec8x32f_cmpltd = vector_comparison_test_bench<vec8x32f, vec8x32f_cmpps_impl>;

    BENCHMARK(comparison_equals_32f::vec8x32f_cmpltd);
    ...
    mask8x32f vec8x32f_vcmpps_impl(vec8x32f lhs, vec8x32f rhs) {
        return mask8x32f{_mm256_cmp_ps_mask(decay(lhs), decay(rhs), _CMP_LT_OS)};
    }
    ...
    mask16x32f vec16x32f_vcmpps_impl(vec16x32f lhs, vec16x32f rhs) {
        return mask16x32f{_mm512_cmp_ps_mask(decay(lhs), decay(rhs), _CMP_LT_OS)};
    }
