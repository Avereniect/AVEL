#ifndef AVEL_comparison_not_equals_32f_HPP
#define AVEL_comparison_not_equals_32f_HPP

namespace avel::benchmarks::comparison_not_equals_32f {
    ...
    BENCHMARK(comparison_not_equals_32f::scalar_native);
    ...
    mask4x32f vec4x32f_cmpps_impl(vec4x32f lhs, vec4x32f rhs) {
        return mask4x32f{_mm_cmpneq_ps(decay(lhs), decay(rhs))};
    }

    auto vec4x32f_cmpps = vector_comparison_test_bench<vec4x32f, vec4x32f_cmpps_impl>;

    BENCHMARK(comparison_not_equals_32f::vec4x32f_cmpps);
    ...
    mask4x32f vec4x32f_cmpeqd_k_impl(vec4x32f lhs, vec4x32f rhs) {
        return mask4x32f{_mm_cmp_ps_mask(decay(lhs), decay(rhs), _CMP_NEQ_OS)};
    }

    auto vec4x32f_cmpeqd_k = vector_comparison_test_bench<vec4x32f, vec4x32f_cmpeqd_k_impl>;

    BENCHMARK(comparison_not_equals_32f::vec4x32f_cmpeqd_k);
    ...
    mask8x32f vec8x32f_cmpps_impl(vec8x32f lhs, vec8x32f rhs) {
        return mask8x32f{_mm256_cmp_ps(decay(lhs), decay(rhs), _CMP_NEQ_OS)};
    }

    auto vec8x32f_cmpeqd = vector_comparison_test_bench<vec8x32f, vec8x32f_cmpps_impl>;

    BENCHMARK(comparison_not_equals_32f::vec8x32f_cmpeqd);
    ...
    mask8x32f vec8x32f_vcmpps_impl(vec8x32f lhs, vec8x32f rhs) {
        return mask8x32f{_mm256_cmp_ps_mask(decay(lhs), decay(rhs), _CMP_NEQ_OS)};
    }

    auto vec8x32f_vcmpps = vector_comparison_test_bench<vec8x32f, vec8x32f_vcmpps_impl>;

    BENCHMARK(comparison_not_equals_32f::vec8x32f_vcmpps);
    ...
    mask16x32f vec16x32f_vcmpps_impl(vec16x32f lhs, vec16x32f rhs) {
        return mask16x32f{_mm512_cmp_ps_mask(decay(lhs), decay(rhs), _CMP_NEQ_OS)};
    }

    auto vec16x32f_vcmpps = vector_comparison_test_bench<vec16x32f, vec16x32f_vcmpps_impl>;

    BENCHMARK(comparison_not_equals_32f::vec16x32f_vcmpps);

    #endif

}

#endif //AVEL_comparison_not_equals_32f_HPP
